...
static void visual_shader_node_vec2_constant_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeVec2Constant"];
...
static JSClassDef visual_shader_node_vec2_constant_class_def = {
    ...
    visual_shader_node_vec2_constant_class_finalizer
};
...
static JSValue visual_shader_node_vec2_constant_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeVec2Constant *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeVec2Constant *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeVec2Constant);
...
static JSValue visual_shader_node_vec2_constant_class_set_constant(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeVec2Constant::set_constant, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_vec2_constant_class_get_constant(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		VisualShaderNodeVec2Constant *obj = static_cast<VisualShaderNodeVec2Constant *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_constant();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		VisualShaderNodeVec2Constant *js_proxy = static_cast<VisualShaderNodeVec2Constant *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_visual_shader_node_vec2_constant_property(JSContext *ctx, JSValue proto) {
    JS_DefinePropertyGetSet(
        ctx,
        proto,
        JS_NewAtom(ctx, "constant"),
        JS_NewCFunction(ctx, visual_shader_node_vec2_constant_class_get_constant, "get_constant", 0),
        JS_NewCFunction(ctx, visual_shader_node_vec2_constant_class_set_constant, "set_constant", 1),
        JS_PROP_GETSET
    );
}

static void define_visual_shader_node_vec2_constant_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_vec2_constant_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeVec2Constant"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeVec2Constant";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_vec2_constant_class_def);
	...
	define_visual_shader_node_vec2_constant_enum(ctx, proto);
...
void __register_visual_shader_node_vec2_constant() {
	js_init_visual_shader_node_vec2_constant_module(js_context());
}

void register_visual_shader_node_vec2_constant() {
	__register_visual_shader_node_vec2_constant();
}