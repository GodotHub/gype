...
static void occluder3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Occluder3D"];
...
static JSValue occluder3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Occluder3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Occluder3D);
...
static void define_occluder3d_property(JSContext *ctx, JSValue proto) {
}

static void define_occluder3d_enum(JSContext *ctx, JSValue proto) {
}

static int js_occluder3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Occluder3D"] = class_id;
	classes_by_id[class_id] = "Occluder3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &occluder3d_class_def);
	...
	define_occluder3d_enum(ctx, proto);
...
void __register_occluder3d() {
	js_init_occluder3d_module(js_context());
}

void register_occluder3d() {
	__register_occluder3d();
}