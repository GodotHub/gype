...
static void placeholder_cubemap_array_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PlaceholderCubemapArray"];
...
static JSClassDef placeholder_cubemap_array_class_def = {
    ...
    placeholder_cubemap_array_class_finalizer
};
...
static JSValue placeholder_cubemap_array_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PlaceholderCubemapArray *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PlaceholderCubemapArray *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PlaceholderCubemapArray);
...
static void define_placeholder_cubemap_array_enum(JSContext *ctx, JSValue ctor) {
}

static int js_placeholder_cubemap_array_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PlaceholderCubemapArray"] = class_id;
	classes_by_id[class_id] = "PlaceholderCubemapArray";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &placeholder_cubemap_array_class_def);
	...
	define_placeholder_cubemap_array_enum(ctx, ctor);
...
void __register_placeholder_cubemap_array() {
	js_init_placeholder_cubemap_array_module(js_context());
}

void register_placeholder_cubemap_array() {
	__register_placeholder_cubemap_array();
}