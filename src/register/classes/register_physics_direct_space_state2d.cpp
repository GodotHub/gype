...
static void physics_direct_space_state2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PhysicsDirectSpaceState2D"];
...
static JSClassDef physics_direct_space_state2d_class_def = {
    ...
    physics_direct_space_state2d_class_finalizer
};
...
static JSValue physics_direct_space_state2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PhysicsDirectSpaceState2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PhysicsDirectSpaceState2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PhysicsDirectSpaceState2D);
...
static void define_physics_direct_space_state2d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_physics_direct_space_state2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PhysicsDirectSpaceState2D"] = class_id;
	classes_by_id[class_id] = "PhysicsDirectSpaceState2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &physics_direct_space_state2d_class_def);
	...
	define_physics_direct_space_state2d_enum(ctx, ctor);
...
void __register_physics_direct_space_state2d() {
	js_init_physics_direct_space_state2d_module(js_context());
}

void register_physics_direct_space_state2d() {
	__register_physics_direct_space_state2d();
}