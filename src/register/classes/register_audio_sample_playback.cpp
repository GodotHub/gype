...
static void audio_sample_playback_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioSamplePlayback"];
...
static JSClassDef audio_sample_playback_class_def = {
    "AudioSamplePlayback",
    audio_sample_playback_class_finalizer
};
...
static JSValue audio_sample_playback_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioSamplePlayback *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioSamplePlayback *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioSamplePlayback);
...
static int js_audio_sample_playback_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AudioSamplePlayback"] = class_id;
	classes_by_id[class_id] = "AudioSamplePlayback";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_sample_playback_class_def);
...
void __register_audio_sample_playback() {
	js_init_audio_sample_playback_module(js_context());
}

void register_audio_sample_playback() {
	__register_audio_sample_playback();
}