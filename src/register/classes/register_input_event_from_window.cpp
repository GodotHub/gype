...
static void input_event_from_window_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["InputEventFromWindow"];
...
static JSClassDef input_event_from_window_class_def = {
    ...
    input_event_from_window_class_finalizer
};
...
static JSValue input_event_from_window_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    InputEventFromWindow *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<InputEventFromWindow *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(InputEventFromWindow);
...
static JSValue input_event_from_window_class_set_window_id(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&InputEventFromWindow::set_window_id, ctx, this_val, argc, argv);
};
...
static void define_input_event_from_window_property(JSContext *ctx, JSValue proto) {
...
static int js_input_event_from_window_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["InputEventFromWindow"] = class_id;
	classes_by_id[class_id] = "InputEventFromWindow";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &input_event_from_window_class_def);
...
void __register_input_event_from_window() {
	js_init_input_event_from_window_module(js_context());
}

void register_input_event_from_window() {
	__register_input_event_from_window();
}