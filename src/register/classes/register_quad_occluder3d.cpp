...
static void quad_occluder3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["QuadOccluder3D"];
...
static JSClassDef quad_occluder3d_class_def = {
    "QuadOccluder3D",
    quad_occluder3d_class_finalizer
};
...
static JSValue quad_occluder3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    QuadOccluder3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<QuadOccluder3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(QuadOccluder3D);
...
static JSValue quad_occluder3d_class_get_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		QuadOccluder3D *obj = static_cast<QuadOccluder3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_size();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		QuadOccluder3D *js_proxy = static_cast<QuadOccluder3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_quad_occluder3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_quad_occluder3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["QuadOccluder3D"] = class_id;
	classes_by_id[class_id] = "QuadOccluder3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &quad_occluder3d_class_def);
	...
	define_quad_occluder3d_enum(ctx, ctor);
...
void __register_quad_occluder3d() {
	js_init_quad_occluder3d_module(js_context());
}

void register_quad_occluder3d() {
	__register_quad_occluder3d();
}