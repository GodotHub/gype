...
static void animation_node_blend3_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AnimationNodeBlend3"];
...
static JSClassDef animation_node_blend3_class_def = {
    "AnimationNodeBlend3",
    animation_node_blend3_class_finalizer
};
...
static JSValue animation_node_blend3_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AnimationNodeBlend3 *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AnimationNodeBlend3 *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AnimationNodeBlend3);
...
static void define_animation_node_blend3_property(JSContext *ctx, JSValue proto) {
}

static void define_animation_node_blend3_enum(JSContext *ctx, JSValue proto) {
}

static int js_animation_node_blend3_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AnimationNodeBlend3"] = class_id;
	classes_by_id[class_id] = "AnimationNodeBlend3";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &animation_node_blend3_class_def);
	...
	define_animation_node_blend3_enum(ctx, proto);
...
void __register_animation_node_blend3() {
	js_init_animation_node_blend3_module(js_context());
}

void register_animation_node_blend3() {
	__register_animation_node_blend3();
}