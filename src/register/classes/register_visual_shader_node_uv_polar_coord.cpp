...
static void visual_shader_node_uv_polar_coord_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeUVPolarCoord"];
...
static JSClassDef visual_shader_node_uv_polar_coord_class_def = {
    ...
    visual_shader_node_uv_polar_coord_class_finalizer
};
...
static JSValue visual_shader_node_uv_polar_coord_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeUVPolarCoord *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeUVPolarCoord *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeUVPolarCoord);
...
static void define_visual_shader_node_uv_polar_coord_property(JSContext *ctx, JSValue proto) {
}

static void define_visual_shader_node_uv_polar_coord_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_uv_polar_coord_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeUVPolarCoord"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeUVPolarCoord";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_uv_polar_coord_class_def);
	...
	define_visual_shader_node_uv_polar_coord_enum(ctx, proto);
...
void __register_visual_shader_node_uv_polar_coord() {
	js_init_visual_shader_node_uv_polar_coord_module(js_context());
}

void register_visual_shader_node_uv_polar_coord() {
	__register_visual_shader_node_uv_polar_coord();
}