...
static void audio_effect_amplify_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectAmplify"];
...
static JSClassDef audio_effect_amplify_class_def = {
    "AudioEffectAmplify",
    audio_effect_amplify_class_finalizer
};
...
static JSValue audio_effect_amplify_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectAmplify *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectAmplify *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectAmplify);
...
static JSValue audio_effect_amplify_class_set_volume_linear(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioEffectAmplify::set_volume_linear, ctx, this_val, argc, argv);
};
static JSValue audio_effect_amplify_class_get_volume_linear(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&AudioEffectAmplify::get_volume_linear, ctx, this_val, argc, argv);
}
...
static const JSCFunctionListEntry audio_effect_amplify_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("set_volume_linear", 1, &audio_effect_amplify_class_set_volume_linear),
	JS_CFUNC_DEF("get_volume_linear", 0, &audio_effect_amplify_class_get_volume_linear),
};
...
static void define_audio_effect_amplify_property(JSContext *ctx, JSValue proto) {
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, audio_effect_amplify_class_get_volume_linear, "get_volume_linear", 0),
        JS_NewCFunction(ctx, audio_effect_amplify_class_set_volume_linear, "set_volume_linear", 1),
        JS_PROP_GETSET
    );
}

static void define_audio_effect_amplify_enum(JSContext *ctx, JSValue ctor) {
}

static int js_audio_effect_amplify_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioEffectAmplify"] = class_id;
	classes_by_id[class_id] = "AudioEffectAmplify";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_amplify_class_def);
	...
	define_audio_effect_amplify_enum(ctx, ctor);
...
void __register_audio_effect_amplify() {
	js_init_audio_effect_amplify_module(js_context());
}

void register_audio_effect_amplify() {
	__register_audio_effect_amplify();
}