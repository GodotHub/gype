...
static void audio_effect_high_pass_filter_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectHighPassFilter"];
...
static JSClassDef audio_effect_high_pass_filter_class_def = {
    ...
    audio_effect_high_pass_filter_class_finalizer
};
...
static JSValue audio_effect_high_pass_filter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectHighPassFilter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectHighPassFilter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectHighPassFilter);
...
static void define_audio_effect_high_pass_filter_enum(JSContext *ctx, JSValue ctor) {
}

static int js_audio_effect_high_pass_filter_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioEffectHighPassFilter"] = class_id;
	classes_by_id[class_id] = "AudioEffectHighPassFilter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_high_pass_filter_class_def);
	...
	define_audio_effect_high_pass_filter_enum(ctx, ctor);
...
void __register_audio_effect_high_pass_filter() {
	js_init_audio_effect_high_pass_filter_module(js_context());
}

void register_audio_effect_high_pass_filter() {
	__register_audio_effect_high_pass_filter();
}