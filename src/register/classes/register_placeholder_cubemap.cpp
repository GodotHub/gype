...
static void placeholder_cubemap_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PlaceholderCubemap"];
...
static JSClassDef placeholder_cubemap_class_def = {
    "PlaceholderCubemap",
    placeholder_cubemap_class_finalizer
};
...
static JSValue placeholder_cubemap_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PlaceholderCubemap *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PlaceholderCubemap *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PlaceholderCubemap);
...
static void define_placeholder_cubemap_enum(JSContext *ctx, JSValue ctor) {
}

static int js_placeholder_cubemap_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PlaceholderCubemap"] = class_id;
	classes_by_id[class_id] = "PlaceholderCubemap";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &placeholder_cubemap_class_def);
	...
	define_placeholder_cubemap_enum(ctx, ctor);
...
void __register_placeholder_cubemap() {
	js_init_placeholder_cubemap_module(js_context());
}

void register_placeholder_cubemap() {
	__register_placeholder_cubemap();
}