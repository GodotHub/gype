...
static void visual_shader_node_boolean_parameter_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeBooleanParameter"];
...
static JSClassDef visual_shader_node_boolean_parameter_class_def = {
    ...
    visual_shader_node_boolean_parameter_class_finalizer
};
...
static JSValue visual_shader_node_boolean_parameter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeBooleanParameter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeBooleanParameter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeBooleanParameter);
...
static int js_visual_shader_node_boolean_parameter_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeBooleanParameter"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeBooleanParameter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_boolean_parameter_class_def);
...
void __register_visual_shader_node_boolean_parameter() {
	js_init_visual_shader_node_boolean_parameter_module(js_context());
}

void register_visual_shader_node_boolean_parameter() {
	__register_visual_shader_node_boolean_parameter();
}