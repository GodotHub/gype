...
static void define_convert_transform_modifier3d_enum(JSContext *ctx, JSValue proto) {
	...
	JS_SetPropertyStr(ctx, proto, "TransformMode", TransformMode_obj);
}
...
static int js_convert_transform_modifier3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_convert_transform_modifier3d_enum(ctx, proto);
