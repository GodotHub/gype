...
static JSValue video_stream_class_get_file(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		VideoStream *obj = static_cast<VideoStream *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_file();
	};
	proxy->setter = [this_val](const String &value) -> void {
		VideoStream *js_proxy = static_cast<VideoStream *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_file(value);
	};
