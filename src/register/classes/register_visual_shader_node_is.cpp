...
static void visual_shader_node_is_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeIs"];
...
static JSClassDef visual_shader_node_is_class_def = {
    "VisualShaderNodeIs",
    visual_shader_node_is_class_finalizer
};
...
static JSValue visual_shader_node_is_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeIs *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeIs *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeIs);
...
static JSValue visual_shader_node_is_class_set_function(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeIs::set_function, ctx, this_val, argc, argv);
};
...
static void define_visual_shader_node_is_property(JSContext *ctx, JSValue proto) {
...
static int js_visual_shader_node_is_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeIs"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeIs";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_is_class_def);
...
void __register_visual_shader_node_is() {
	js_init_visual_shader_node_is_module(js_context());
}

void register_visual_shader_node_is() {
	__register_visual_shader_node_is();
}