...
static void animation_node_output_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AnimationNodeOutput"];
...
static JSClassDef animation_node_output_class_def = {
    "AnimationNodeOutput",
    animation_node_output_class_finalizer
};
...
static JSValue animation_node_output_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AnimationNodeOutput *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AnimationNodeOutput *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AnimationNodeOutput);
...
static void define_animation_node_output_enum(JSContext *ctx, JSValue ctor) {
}

static int js_animation_node_output_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AnimationNodeOutput"] = class_id;
	classes_by_id[class_id] = "AnimationNodeOutput";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &animation_node_output_class_def);
	...
	define_animation_node_output_enum(ctx, ctor);
...
void __register_animation_node_output() {
	js_init_animation_node_output_module(js_context());
}

void register_animation_node_output() {
	__register_animation_node_output();
}