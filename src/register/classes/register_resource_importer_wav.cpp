...
static void resource_importer_wav_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ResourceImporterWAV"];
...
static JSClassDef resource_importer_wav_class_def = {
    "ResourceImporterWAV",
    resource_importer_wav_class_finalizer
};
...
static JSValue resource_importer_wav_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ResourceImporterWAV *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ResourceImporterWAV *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ResourceImporterWAV);
...
static void define_resource_importer_wav_enum(JSContext *ctx, JSValue ctor) {
}

static int js_resource_importer_wav_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ResourceImporterWAV"] = class_id;
	classes_by_id[class_id] = "ResourceImporterWAV";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &resource_importer_wav_class_def);
	...
	define_resource_importer_wav_enum(ctx, ctor);
...
void __register_resource_importer_wav() {
	js_init_resource_importer_wav_module(js_context());
}

void register_resource_importer_wav() {
	__register_resource_importer_wav();
}