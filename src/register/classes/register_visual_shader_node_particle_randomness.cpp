...
static void visual_shader_node_particle_randomness_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeParticleRandomness"];
...
static JSClassDef visual_shader_node_particle_randomness_class_def = {
    ...
    visual_shader_node_particle_randomness_class_finalizer
};
...
static JSValue visual_shader_node_particle_randomness_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeParticleRandomness *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeParticleRandomness *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeParticleRandomness);
...
static JSValue visual_shader_node_particle_randomness_class_set_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeParticleRandomness::set_op_type, ctx, this_val, argc, argv);
};
...
static void define_visual_shader_node_particle_randomness_property(JSContext *ctx, JSValue proto) {
...
static void define_visual_shader_node_particle_randomness_enum(JSContext *ctx, JSValue proto) {
...
static int js_visual_shader_node_particle_randomness_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeParticleRandomness"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeParticleRandomness";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_particle_randomness_class_def);
	...
	define_visual_shader_node_particle_randomness_enum(ctx, proto);
...
void __register_visual_shader_node_particle_randomness() {
	js_init_visual_shader_node_particle_randomness_module(js_context());
}

void register_visual_shader_node_particle_randomness() {
	__register_visual_shader_node_particle_randomness();
}