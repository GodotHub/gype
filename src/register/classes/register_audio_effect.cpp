...
static void audio_effect_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffect"];
...
static JSValue audio_effect_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<AudioEffect *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffect);
...
static int js_audio_effect_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AudioEffect"] = class_id;
	classes_by_id[class_id] = "AudioEffect";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_class_def);
...
void __register_audio_effect() {
	js_init_audio_effect_module(js_context());
}

void register_audio_effect() {
	__register_audio_effect();
}