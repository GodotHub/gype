...
static void gltf_document_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["GLTFDocumentExtension"];
...
static JSClassDef gltf_document_extension_class_def = {
    ...
    gltf_document_extension_class_finalizer
};
...
static JSValue gltf_document_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    GLTFDocumentExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<GLTFDocumentExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(GLTFDocumentExtension);
...
static void define_gltf_document_extension_property(JSContext *ctx, JSValue proto) {
}

static void define_gltf_document_extension_enum(JSContext *ctx, JSValue ctor) {
}

static int js_gltf_document_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["GLTFDocumentExtension"] = class_id;
	classes_by_id[class_id] = "GLTFDocumentExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &gltf_document_extension_class_def);
	...
	define_gltf_document_extension_enum(ctx, ctor);
...
void __register_gltf_document_extension() {
	js_init_gltf_document_extension_module(js_context());
}

void register_gltf_document_extension() {
	__register_gltf_document_extension();
}