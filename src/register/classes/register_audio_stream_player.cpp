...
static JSValue audio_stream_player_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<AudioStreamPlayer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue audio_stream_player_class_get_bus(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> StringName {
		AudioStreamPlayer *obj = static_cast<AudioStreamPlayer *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_bus();
	};
	proxy->setter = [this_val](const StringName &value) -> void {
		AudioStreamPlayer *js_proxy = static_cast<AudioStreamPlayer *>(VariantAdapter(this_val).get().operator Object *());
...
static JSValue audio_stream_player_class_get_finished_signal(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	AudioStreamPlayer *opaque = static_cast<AudioStreamPlayer *>(reinterpret_cast<VariantAdapter *>(JS_GetOpaque(this_val, JS_GetClassID(this_val)))->get().operator Object*());
