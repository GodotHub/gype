...
static void csg_combiner3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CSGCombiner3D"];
...
static JSClassDef csg_combiner3d_class_def = {
    "CSGCombiner3D",
    csg_combiner3d_class_finalizer
};
...
static JSValue csg_combiner3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<CSGCombiner3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CSGCombiner3D);
...
static void define_csg_combiner3d_enum(JSContext *ctx, JSValue proto) {
}

static int js_csg_combiner3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CSGCombiner3D"] = class_id;
	classes_by_id[class_id] = "CSGCombiner3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &csg_combiner3d_class_def);
	...
	define_csg_combiner3d_enum(ctx, proto);
...
void __register_csg_combiner3d() {
	js_init_csg_combiner3d_module(js_context());
}

void register_csg_combiner3d() {
	__register_csg_combiner3d();
}