...
static void csg_primitive3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CSGPrimitive3D"];
...
static JSClassDef csg_primitive3d_class_def = {
    "CSGPrimitive3D",
    csg_primitive3d_class_finalizer
};
...
static JSValue csg_primitive3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CSGPrimitive3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CSGPrimitive3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CSGPrimitive3D);
...
static void define_csg_primitive3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_csg_primitive3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CSGPrimitive3D"] = class_id;
	classes_by_id[class_id] = "CSGPrimitive3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &csg_primitive3d_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["CSGShape3D"]);
	...
	define_csg_primitive3d_enum(ctx, ctor);
...
void __register_csg_primitive3d() {
	js_init_csg_primitive3d_module(js_context());
}

void register_csg_primitive3d() {
	__register_csg_primitive3d();
}