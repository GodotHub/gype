...
static void text_server_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["TextServerExtension"];
...
static JSClassDef text_server_extension_class_def = {
    "TextServerExtension",
    text_server_extension_class_finalizer
};
...
static JSValue text_server_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    TextServerExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<TextServerExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(TextServerExtension);
...
static void define_text_server_extension_enum(JSContext *ctx, JSValue ctor) {
}

static int js_text_server_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["TextServerExtension"] = class_id;
	classes_by_id[class_id] = "TextServerExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &text_server_extension_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["TextServer"]);
	...
	define_text_server_extension_enum(ctx, ctor);
...
void __register_text_server_extension() {
	js_init_text_server_extension_module(js_context());
}

void register_text_server_extension() {
	__register_text_server_extension();
}