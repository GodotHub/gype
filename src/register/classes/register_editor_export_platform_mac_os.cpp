...
static void editor_export_platform_mac_os_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorExportPlatformMacOS"];
...
static JSClassDef editor_export_platform_mac_os_class_def = {
    ...
    editor_export_platform_mac_os_class_finalizer
};
...
static JSValue editor_export_platform_mac_os_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorExportPlatformMacOS *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorExportPlatformMacOS *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorExportPlatformMacOS);
...
static void define_editor_export_platform_mac_os_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_export_platform_mac_os_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorExportPlatformMacOS"] = class_id;
	classes_by_id[class_id] = "EditorExportPlatformMacOS";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_export_platform_mac_os_class_def);
	...
	define_editor_export_platform_mac_os_enum(ctx, ctor);
...
void __register_editor_export_platform_mac_os() {
	js_init_editor_export_platform_mac_os_module(js_context());
}

void register_editor_export_platform_mac_os() {
	__register_editor_export_platform_mac_os();
}