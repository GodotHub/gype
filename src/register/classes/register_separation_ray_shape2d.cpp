...
static void define_separation_ray_shape2d_enum(JSContext *ctx, JSValue ctor) {
}
...
static int js_separation_ray_shape2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_separation_ray_shape2d_enum(ctx, ctor);
	JS_SetConstructor(ctx, ctor, proto);
	JS_SetModuleExport(ctx, m, "SeparationRayShape2D", ctor);
	ctor_list["SeparationRayShape2D"] = ctor;
