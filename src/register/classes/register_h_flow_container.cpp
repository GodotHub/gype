...
static void h_flow_container_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["HFlowContainer"];
...
static JSClassDef h_flow_container_class_def = {
    "HFlowContainer",
    h_flow_container_class_finalizer
};
...
static JSValue h_flow_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    HFlowContainer *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<HFlowContainer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(HFlowContainer);
...
static void define_h_flow_container_property(JSContext *ctx, JSValue proto) {
}

static void define_h_flow_container_enum(JSContext *ctx, JSValue proto) {
}

static int js_h_flow_container_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["HFlowContainer"] = class_id;
	classes_by_id[class_id] = "HFlowContainer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &h_flow_container_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["FlowContainer"]);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, class_id, proto);

	define_h_flow_container_property(ctx, proto);
	define_h_flow_container_enum(ctx, proto);
...
void __register_h_flow_container() {
	js_init_h_flow_container_module(js_context());
}

void register_h_flow_container() {
	__register_h_flow_container();
}