...
static void instance_placeholder_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["InstancePlaceholder"];
...
static JSClassDef instance_placeholder_class_def = {
    "InstancePlaceholder",
    instance_placeholder_class_finalizer
};
...
static JSValue instance_placeholder_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<InstancePlaceholder *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(InstancePlaceholder);
...
static int js_instance_placeholder_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["InstancePlaceholder"] = class_id;
	classes_by_id[class_id] = "InstancePlaceholder";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &instance_placeholder_class_def);
...
void __register_instance_placeholder() {
	js_init_instance_placeholder_module(js_context());
}

void register_instance_placeholder() {
	__register_instance_placeholder();
}