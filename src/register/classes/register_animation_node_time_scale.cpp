...
static void animation_node_time_scale_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AnimationNodeTimeScale"];
...
static JSClassDef animation_node_time_scale_class_def = {
    ...
    animation_node_time_scale_class_finalizer
};
...
static JSValue animation_node_time_scale_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AnimationNodeTimeScale *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AnimationNodeTimeScale *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AnimationNodeTimeScale);
...
static void define_animation_node_time_scale_enum(JSContext *ctx, JSValue ctor) {
}

static int js_animation_node_time_scale_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AnimationNodeTimeScale"] = class_id;
	classes_by_id[class_id] = "AnimationNodeTimeScale";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &animation_node_time_scale_class_def);
	...
	define_animation_node_time_scale_enum(ctx, ctor);
...
void __register_animation_node_time_scale() {
	js_init_animation_node_time_scale_module(js_context());
}

void register_animation_node_time_scale() {
	__register_animation_node_time_scale();
}