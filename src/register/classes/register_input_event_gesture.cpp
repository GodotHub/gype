...
static void input_event_gesture_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["InputEventGesture"];
...
static JSClassDef input_event_gesture_class_def = {
    "InputEventGesture",
    input_event_gesture_class_finalizer
};
...
static JSValue input_event_gesture_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    InputEventGesture *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<InputEventGesture *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(InputEventGesture);
...
static JSValue input_event_gesture_class_get_position(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		InputEventGesture *obj = static_cast<InputEventGesture *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_position();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		InputEventGesture *js_proxy = static_cast<InputEventGesture *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_input_event_gesture_enum(JSContext *ctx, JSValue ctor) {
}

static int js_input_event_gesture_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["InputEventGesture"] = class_id;
	classes_by_id[class_id] = "InputEventGesture";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &input_event_gesture_class_def);
	...
	define_input_event_gesture_enum(ctx, ctor);
...
void __register_input_event_gesture() {
	js_init_input_event_gesture_module(js_context());
}

void register_input_event_gesture() {
	__register_input_event_gesture();
}