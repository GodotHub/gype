...
static void texture_cubemap_rd_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["TextureCubemapRD"];
...
static JSClassDef texture_cubemap_rd_class_def = {
    "TextureCubemapRD",
    texture_cubemap_rd_class_finalizer
};
...
static JSValue texture_cubemap_rd_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    TextureCubemapRD *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<TextureCubemapRD *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(TextureCubemapRD);
...
static void define_texture_cubemap_rd_enum(JSContext *ctx, JSValue ctor) {
}

static int js_texture_cubemap_rd_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["TextureCubemapRD"] = class_id;
	classes_by_id[class_id] = "TextureCubemapRD";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &texture_cubemap_rd_class_def);
	...
	define_texture_cubemap_rd_enum(ctx, ctor);
...
void __register_texture_cubemap_rd() {
	js_init_texture_cubemap_rd_module(js_context());
}

void register_texture_cubemap_rd() {
	__register_texture_cubemap_rd();
}