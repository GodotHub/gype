...
static void texture_layered_rd_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["TextureLayeredRD"];
...
static JSClassDef texture_layered_rd_class_def = {
    "TextureLayeredRD",
    texture_layered_rd_class_finalizer
};
...
static JSValue texture_layered_rd_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    TextureLayeredRD *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<TextureLayeredRD *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(TextureLayeredRD);
...
static JSValue texture_layered_rd_class_get_texture_rd_rid(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> RID {
		TextureLayeredRD *obj = static_cast<TextureLayeredRD *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_texture_rd_rid();
	};
	proxy->setter = [this_val](const RID &value) -> void {
		TextureLayeredRD *js_proxy = static_cast<TextureLayeredRD *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_texture_layered_rd_enum(JSContext *ctx, JSValue ctor) {
}

static int js_texture_layered_rd_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["TextureLayeredRD"] = class_id;
	classes_by_id[class_id] = "TextureLayeredRD";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &texture_layered_rd_class_def);
	...
	define_texture_layered_rd_enum(ctx, ctor);
...
void __register_texture_layered_rd() {
	js_init_texture_layered_rd_module(js_context());
}

void register_texture_layered_rd() {
	__register_texture_layered_rd();
}