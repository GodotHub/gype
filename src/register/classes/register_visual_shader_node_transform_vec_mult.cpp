...
static void visual_shader_node_transform_vec_mult_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeTransformVecMult"];
...
static JSClassDef visual_shader_node_transform_vec_mult_class_def = {
    ...
    visual_shader_node_transform_vec_mult_class_finalizer
};
...
static JSValue visual_shader_node_transform_vec_mult_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeTransformVecMult *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeTransformVecMult *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeTransformVecMult);
...
static JSValue visual_shader_node_transform_vec_mult_class_set_operator(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeTransformVecMult::set_operator, ctx, this_val, argc, argv);
};
...
static void define_visual_shader_node_transform_vec_mult_property(JSContext *ctx, JSValue proto) {
...
static void define_visual_shader_node_transform_vec_mult_enum(JSContext *ctx, JSValue proto) {
	...
	JS_SetPropertyStr(ctx, Operator_obj, "OP_3x3_AxB", JS_NewInt64(ctx, 2));
	JS_SetPropertyStr(ctx, Operator_obj, "OP_3x3_BxA", JS_NewInt64(ctx, 3));
	JS_SetPropertyStr(ctx, Operator_obj, "OP_MAX", JS_NewInt64(ctx, 4));
...
static int js_visual_shader_node_transform_vec_mult_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeTransformVecMult"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeTransformVecMult";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_transform_vec_mult_class_def);
	...
	define_visual_shader_node_transform_vec_mult_enum(ctx, proto);
...
void __register_visual_shader_node_transform_vec_mult() {
	js_init_visual_shader_node_transform_vec_mult_module(js_context());
}

void register_visual_shader_node_transform_vec_mult() {
	__register_visual_shader_node_transform_vec_mult();
}