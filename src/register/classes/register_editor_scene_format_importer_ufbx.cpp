...
static void editor_scene_format_importer_ufbx_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorSceneFormatImporterUFBX"];
...
static JSClassDef editor_scene_format_importer_ufbx_class_def = {
    ...
    editor_scene_format_importer_ufbx_class_finalizer
};
...
static JSValue editor_scene_format_importer_ufbx_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorSceneFormatImporterUFBX *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorSceneFormatImporterUFBX *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorSceneFormatImporterUFBX);
...
static void define_editor_scene_format_importer_ufbx_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_scene_format_importer_ufbx_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorSceneFormatImporterUFBX"] = class_id;
	classes_by_id[class_id] = "EditorSceneFormatImporterUFBX";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_scene_format_importer_ufbx_class_def);
	...
	define_editor_scene_format_importer_ufbx_enum(ctx, ctor);
...
void __register_editor_scene_format_importer_ufbx() {
	js_init_editor_scene_format_importer_ufbx_module(js_context());
}

void register_editor_scene_format_importer_ufbx() {
	__register_editor_scene_format_importer_ufbx();
}