...
static void open_xr_composition_layer_quad_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["OpenXRCompositionLayerQuad"];
...
static JSClassDef open_xr_composition_layer_quad_class_def = {
    ...
    open_xr_composition_layer_quad_class_finalizer
};
...
static JSValue open_xr_composition_layer_quad_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    OpenXRCompositionLayerQuad *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<OpenXRCompositionLayerQuad *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(OpenXRCompositionLayerQuad);
...
static JSValue open_xr_composition_layer_quad_class_set_quad_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&OpenXRCompositionLayerQuad::set_quad_size, ctx, this_val, argc, argv);
};
static JSValue open_xr_composition_layer_quad_class_get_quad_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		OpenXRCompositionLayerQuad *obj = static_cast<OpenXRCompositionLayerQuad *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_quad_size();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		OpenXRCompositionLayerQuad *js_proxy = static_cast<OpenXRCompositionLayerQuad *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_quad_size(value);
	};
...
static void define_open_xr_composition_layer_quad_property(JSContext *ctx, JSValue proto) {
...
static void define_open_xr_composition_layer_quad_enum(JSContext *ctx, JSValue proto) {
}

static int js_open_xr_composition_layer_quad_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["OpenXRCompositionLayerQuad"] = class_id;
	classes_by_id[class_id] = "OpenXRCompositionLayerQuad";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &open_xr_composition_layer_quad_class_def);
	...
	define_open_xr_composition_layer_quad_enum(ctx, proto);
...
void __register_open_xr_composition_layer_quad() {
	js_init_open_xr_composition_layer_quad_module(js_context());
}

void register_open_xr_composition_layer_quad() {
	__register_open_xr_composition_layer_quad();
}