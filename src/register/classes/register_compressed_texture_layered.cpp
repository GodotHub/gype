...
static JSValue compressed_texture_layered_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CompressedTextureLayered *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CompressedTextureLayered *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue compressed_texture_layered_class_get_load_path(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		CompressedTextureLayered *obj = static_cast<CompressedTextureLayered *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_load_path();
	};
	proxy->setter = [this_val](const String &value) -> void {
		CompressedTextureLayered *js_proxy = static_cast<CompressedTextureLayered *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->load(value);
	};
