...
static void tweener_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Tweener"];
...
static JSValue tweener_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Tweener *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Tweener);
...
static JSValue tweener_class_get_finished_signal(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	Tweener *opaque = static_cast<Tweener *>(reinterpret_cast<VariantAdapter *>(JS_GetOpaque(this_val, JS_GetClassID(this_val)))->get().operator Object*());
...
static int js_tweener_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["Tweener"] = class_id;
	classes_by_id[class_id] = "Tweener";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &tweener_class_def);
...
void __register_tweener() {
	js_init_tweener_module(js_context());
}
