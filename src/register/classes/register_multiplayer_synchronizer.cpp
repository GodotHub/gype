...
static JSValue multiplayer_synchronizer_class_get_root_path(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> NodePath {
		MultiplayerSynchronizer *obj = static_cast<MultiplayerSynchronizer *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_root_path();
	};
	proxy->setter = [this_val](const NodePath &value) -> void {
		MultiplayerSynchronizer *js_proxy = static_cast<MultiplayerSynchronizer *>(VariantAdapter(this_val).get().operator Object *());
