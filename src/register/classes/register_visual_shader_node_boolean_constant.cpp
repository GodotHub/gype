...
static void visual_shader_node_boolean_constant_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeBooleanConstant"];
...
static JSClassDef visual_shader_node_boolean_constant_class_def = {
    ...
    visual_shader_node_boolean_constant_class_finalizer
};
...
static JSValue visual_shader_node_boolean_constant_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeBooleanConstant *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeBooleanConstant *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeBooleanConstant);
...
static JSValue visual_shader_node_boolean_constant_class_set_constant(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeBooleanConstant::set_constant, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_boolean_constant_class_get_constant(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeBooleanConstant::get_constant, ctx, this_val, argc, argv);
}
...
static void define_visual_shader_node_boolean_constant_property(JSContext *ctx, JSValue proto) {
    JS_DefinePropertyGetSet(
        ctx,
        proto,
        JS_NewAtom(ctx, "constant"),
        JS_NewCFunction(ctx, visual_shader_node_boolean_constant_class_get_constant, "get_constant", 0),
        JS_NewCFunction(ctx, visual_shader_node_boolean_constant_class_set_constant, "set_constant", 1),
        JS_PROP_GETSET
    );
}

static void define_visual_shader_node_boolean_constant_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_boolean_constant_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeBooleanConstant"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeBooleanConstant";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_boolean_constant_class_def);
	...
	define_visual_shader_node_boolean_constant_property(ctx, proto);
	define_visual_shader_node_boolean_constant_enum(ctx, proto);
...
void __register_visual_shader_node_boolean_constant() {
	js_init_visual_shader_node_boolean_constant_module(js_context());
}

void register_visual_shader_node_boolean_constant() {
	__register_visual_shader_node_boolean_constant();
}