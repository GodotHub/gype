...
static JSClassDef editor_resource_preview_generator_class_def = {
    ...
    editor_resource_preview_generator_class_finalizer
};
...
static void define_editor_resource_preview_generator_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_resource_preview_generator_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorResourcePreviewGenerator"] = class_id;
	classes_by_id[class_id] = "EditorResourcePreviewGenerator";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_resource_preview_generator_class_def);
...
void __register_editor_resource_preview_generator() {
	js_init_editor_resource_preview_generator_module(js_context());
}

void register_editor_resource_preview_generator() {
	__register_editor_resource_preview_generator();
}