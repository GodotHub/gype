...
static void visual_shader_node_smooth_step_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeSmoothStep"];
...
static JSClassDef visual_shader_node_smooth_step_class_def = {
    ...
    visual_shader_node_smooth_step_class_finalizer
};
...
static JSValue visual_shader_node_smooth_step_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeSmoothStep *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeSmoothStep *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeSmoothStep);
...
static void define_visual_shader_node_smooth_step_enum(JSContext *ctx, JSValue ctor) {
...
static int js_visual_shader_node_smooth_step_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeSmoothStep"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeSmoothStep";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_smooth_step_class_def);
	...
	define_visual_shader_node_smooth_step_enum(ctx, ctor);
...
void __register_visual_shader_node_smooth_step() {
	js_init_visual_shader_node_smooth_step_module(js_context());
}

void register_visual_shader_node_smooth_step() {
	__register_visual_shader_node_smooth_step();
}