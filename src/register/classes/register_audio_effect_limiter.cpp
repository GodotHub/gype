...
static void audio_effect_limiter_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectLimiter"];
...
static JSClassDef audio_effect_limiter_class_def = {
    "AudioEffectLimiter",
    audio_effect_limiter_class_finalizer
};
...
static JSValue audio_effect_limiter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectLimiter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectLimiter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectLimiter);
...
static JSValue audio_effect_limiter_class_set_ceiling_db(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&AudioEffectLimiter::set_ceiling_db, ctx, this_val, argc, argv);
};
...
static JSValue audio_effect_limiter_class_set_threshold_db(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioEffectLimiter::set_threshold_db, ctx, this_val, argc, argv);
};
...
static JSValue audio_effect_limiter_class_set_soft_clip_db(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioEffectLimiter::set_soft_clip_db, ctx, this_val, argc, argv);
};
...
static JSValue audio_effect_limiter_class_set_soft_clip_ratio(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioEffectLimiter::set_soft_clip_ratio, ctx, this_val, argc, argv);
};
...
static void define_audio_effect_limiter_property(JSContext *ctx, JSValue proto) {
...
static void define_audio_effect_limiter_enum(JSContext *ctx, JSValue proto) {
}

static int js_audio_effect_limiter_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioEffectLimiter"] = class_id;
	classes_by_id[class_id] = "AudioEffectLimiter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_limiter_class_def);
	...
	define_audio_effect_limiter_enum(ctx, proto);
...
void __register_audio_effect_limiter() {
	js_init_audio_effect_limiter_module(js_context());
}

void register_audio_effect_limiter() {
	__register_audio_effect_limiter();
}