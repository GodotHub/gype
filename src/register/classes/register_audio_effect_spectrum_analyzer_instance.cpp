...
static void audio_effect_spectrum_analyzer_instance_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectSpectrumAnalyzerInstance"];
...
static JSClassDef audio_effect_spectrum_analyzer_instance_class_def = {
    ...
    audio_effect_spectrum_analyzer_instance_class_finalizer
};
...
static JSValue audio_effect_spectrum_analyzer_instance_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectSpectrumAnalyzerInstance *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectSpectrumAnalyzerInstance *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectSpectrumAnalyzerInstance);
...
static int js_audio_effect_spectrum_analyzer_instance_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AudioEffectSpectrumAnalyzerInstance"] = class_id;
	classes_by_id[class_id] = "AudioEffectSpectrumAnalyzerInstance";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_spectrum_analyzer_instance_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["AudioEffectInstance"]);
...
void __register_audio_effect_spectrum_analyzer_instance() {
	js_init_audio_effect_spectrum_analyzer_instance_module(js_context());
}

void register_audio_effect_spectrum_analyzer_instance() {
	__register_audio_effect_spectrum_analyzer_instance();
}