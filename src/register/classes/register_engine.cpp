...
static JSValue engine_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	JSClassID class_id = classes["_Engine"];
	...
	VariantAdapter *adapter = memnew(VariantAdapter(engine_class));
...
static int js_engine_class_init(JSContext *ctx, JSModuleDef *m){
	...
	JS_SetModuleExport(ctx, m, "Engine", singleton);
...
void register_engine() {
	_js_init_engine_module(js_context(), "@godot/classes/engine");
}