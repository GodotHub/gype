...
static void texture3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Texture3D"];
...
static JSValue texture3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Texture3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Texture3D);
...
static void define_texture3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_texture3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Texture3D"] = class_id;
	classes_by_id[class_id] = "Texture3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &texture3d_class_def);
	...
	define_texture3d_enum(ctx, ctor);
...
void __register_texture3d() {
	js_init_texture3d_module(js_context());
}

void register_texture3d() {
	__register_texture3d();
}