...
static void panel_container_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PanelContainer"];
...
static JSClassDef panel_container_class_def = {
    "PanelContainer",
    panel_container_class_finalizer
};
...
static JSValue panel_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PanelContainer *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PanelContainer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PanelContainer);
...
static void define_panel_container_enum(JSContext *ctx, JSValue proto) {
}

static int js_panel_container_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PanelContainer"] = class_id;
	classes_by_id[class_id] = "PanelContainer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &panel_container_class_def);
	...
	define_panel_container_enum(ctx, proto);
...
void __register_panel_container() {
	js_init_panel_container_module(js_context());
}

void register_panel_container() {
	__register_panel_container();
}