...
static void audio_effect_band_limit_filter_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectBandLimitFilter"];
...
static JSClassDef audio_effect_band_limit_filter_class_def = {
    ...
    audio_effect_band_limit_filter_class_finalizer
};
...
static JSValue audio_effect_band_limit_filter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectBandLimitFilter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectBandLimitFilter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectBandLimitFilter);
...
static void define_audio_effect_band_limit_filter_enum(JSContext *ctx, JSValue ctor) {
}

static int js_audio_effect_band_limit_filter_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioEffectBandLimitFilter"] = class_id;
	classes_by_id[class_id] = "AudioEffectBandLimitFilter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_band_limit_filter_class_def);
	...
	define_audio_effect_band_limit_filter_enum(ctx, ctor);
...
void __register_audio_effect_band_limit_filter() {
	js_init_audio_effect_band_limit_filter_module(js_context());
}

void register_audio_effect_band_limit_filter() {
	__register_audio_effect_band_limit_filter();
}