...
static void static_body3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["StaticBody3D"];
...
static JSValue static_body3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<StaticBody3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(StaticBody3D);
...
static JSValue static_body3d_class_set_constant_linear_velocity(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&StaticBody3D::set_constant_linear_velocity, ctx, this_val, argc, argv);
};
static JSValue static_body3d_class_set_constant_angular_velocity(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&StaticBody3D::set_constant_angular_velocity, ctx, this_val, argc, argv);
};
static JSValue static_body3d_class_get_constant_linear_velocity(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		StaticBody3D *obj = static_cast<StaticBody3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_constant_linear_velocity();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		StaticBody3D *js_proxy = static_cast<StaticBody3D *>(VariantAdapter(this_val).get().operator Object *());
...
static JSValue static_body3d_class_get_constant_angular_velocity(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		StaticBody3D *obj = static_cast<StaticBody3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_constant_angular_velocity();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		StaticBody3D *js_proxy = static_cast<StaticBody3D *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_constant_angular_velocity(value);
	};
	JSValue obj = JS_NewObjectClass(ctx, classes["Vector3Proxy"]);
	if (is_exception(ctx, obj)) {
		return JS_EXCEPTION;
...
static JSValue static_body3d_class_set_physics_material_override(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&StaticBody3D::set_physics_material_override, ctx, this_val, argc, argv);
};
...
static void define_static_body3d_property(JSContext *ctx, JSValue proto) {
...
static void define_static_body3d_enum(JSContext *ctx, JSValue proto) {
}

static int js_static_body3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["StaticBody3D"] = class_id;
	classes_by_id[class_id] = "StaticBody3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &static_body3d_class_def);
	...
	define_static_body3d_enum(ctx, proto);
...
void __register_static_body3d() {
	js_init_static_body3d_module(js_context());
}

void register_static_body3d() {
	__register_static_body3d();
}