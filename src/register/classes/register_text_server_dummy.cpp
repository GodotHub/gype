...
static void text_server_dummy_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["TextServerDummy"];
...
static JSClassDef text_server_dummy_class_def = {
    "TextServerDummy",
    text_server_dummy_class_finalizer
};
...
static JSValue text_server_dummy_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    TextServerDummy *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<TextServerDummy *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(TextServerDummy);
...
static void define_text_server_dummy_property(JSContext *ctx, JSValue proto) {
}

static void define_text_server_dummy_enum(JSContext *ctx, JSValue proto) {
}

static int js_text_server_dummy_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["TextServerDummy"] = class_id;
	classes_by_id[class_id] = "TextServerDummy";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &text_server_dummy_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["TextServerExtension"]);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, class_id, proto);

	define_text_server_dummy_property(ctx, proto);
	define_text_server_dummy_enum(ctx, proto);
...
void __register_text_server_dummy() {
	js_init_text_server_dummy_module(js_context());
}

void register_text_server_dummy() {
	__register_text_server_dummy();
}