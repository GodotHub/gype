...
static void point_mesh_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PointMesh"];
...
static JSValue point_mesh_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<PointMesh *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PointMesh);
...
static void define_point_mesh_enum(JSContext *ctx, JSValue ctor) {
}

static int js_point_mesh_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PointMesh"] = class_id;
	classes_by_id[class_id] = "PointMesh";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &point_mesh_class_def);
	...
	define_point_mesh_enum(ctx, ctor);
...
void __register_point_mesh() {
	js_init_point_mesh_module(js_context());
}

void register_point_mesh() {
	__register_point_mesh();
}