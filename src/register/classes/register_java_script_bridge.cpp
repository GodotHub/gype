...
static JSValue java_script_bridge_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	JSClassID class_id = classes["_JavaScriptBridge"];
	...
	VariantAdapter *adapter = memnew(VariantAdapter(java_script_bridge_class));
...
static int js_java_script_bridge_class_init(JSContext *ctx, JSModuleDef *m){
	...
	classes["_JavaScriptBridge"] = class_id;
	...
	JS_SetModuleExport(ctx, m, "JavaScriptBridge", singleton);
...
void register_java_script_bridge() {
	_js_init_java_script_bridge_module(js_context(), "@godot/classes/java_script_bridge");
}