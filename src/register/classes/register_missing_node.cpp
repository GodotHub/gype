...
static void missing_node_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["MissingNode"];
...
static JSValue missing_node_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    MissingNode *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<MissingNode *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(MissingNode);
...
static JSValue missing_node_class_get_original_class(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		MissingNode *obj = static_cast<MissingNode *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_original_class();
	};
	proxy->setter = [this_val](const String &value) -> void {
		MissingNode *js_proxy = static_cast<MissingNode *>(VariantAdapter(this_val).get().operator Object *());
...
static JSValue missing_node_class_get_original_scene(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		MissingNode *obj = static_cast<MissingNode *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_original_scene();
	};
	proxy->setter = [this_val](const String &value) -> void {
		MissingNode *js_proxy = static_cast<MissingNode *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_original_scene(value);
	};
	JSValue obj = JS_NewObjectClass(ctx, classes["StringProxy"]);
	if (is_exception(ctx, obj)) {
		return JS_EXCEPTION;
...
static int js_missing_node_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["MissingNode"] = class_id;
	classes_by_id[class_id] = "MissingNode";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &missing_node_class_def);
...
void __register_missing_node() {
	js_init_missing_node_module(js_context());
}

void register_missing_node() {
	__register_missing_node();
}