...
static JSValue shader_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Shader *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue shader_class_get_code(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		Shader *obj = static_cast<Shader *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_code();
	};
	proxy->setter = [this_val](const String &value) -> void {
		Shader *js_proxy = static_cast<Shader *>(VariantAdapter(this_val).get().operator Object *());
