...
static void callback_tweener_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CallbackTweener"];
...
static JSClassDef callback_tweener_class_def = {
    "CallbackTweener",
    callback_tweener_class_finalizer
};
...
static JSValue callback_tweener_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CallbackTweener *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CallbackTweener *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CallbackTweener);
...
static void define_callback_tweener_enum(JSContext *ctx, JSValue ctor) {
}

static int js_callback_tweener_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CallbackTweener"] = class_id;
	classes_by_id[class_id] = "CallbackTweener";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &callback_tweener_class_def);
	...
	define_callback_tweener_enum(ctx, ctor);
...
void __register_callback_tweener() {
	js_init_callback_tweener_module(js_context());
}

void register_callback_tweener() {
	__register_callback_tweener();
}