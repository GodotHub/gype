...
static void array_occluder3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ArrayOccluder3D"];
...
static JSClassDef array_occluder3d_class_def = {
    "ArrayOccluder3D",
    array_occluder3d_class_finalizer
};
...
static JSValue array_occluder3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ArrayOccluder3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ArrayOccluder3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ArrayOccluder3D);
...
static void define_array_occluder3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_array_occluder3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ArrayOccluder3D"] = class_id;
	classes_by_id[class_id] = "ArrayOccluder3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &array_occluder3d_class_def);
	...
	define_array_occluder3d_enum(ctx, ctor);
...
void __register_array_occluder3d() {
	js_init_array_occluder3d_module(js_context());
}

void register_array_occluder3d() {
	__register_array_occluder3d();
}