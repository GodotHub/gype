...
static void compressed_texture2d_array_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CompressedTexture2DArray"];
...
static JSClassDef compressed_texture2d_array_class_def = {
    ...
    compressed_texture2d_array_class_finalizer
};
...
static JSValue compressed_texture2d_array_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CompressedTexture2DArray *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CompressedTexture2DArray *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CompressedTexture2DArray);
...
static void define_compressed_texture2d_array_enum(JSContext *ctx, JSValue ctor) {
}

static int js_compressed_texture2d_array_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CompressedTexture2DArray"] = class_id;
	classes_by_id[class_id] = "CompressedTexture2DArray";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &compressed_texture2d_array_class_def);
	...
	define_compressed_texture2d_array_enum(ctx, ctor);
...
void __register_compressed_texture2d_array() {
	js_init_compressed_texture2d_array_module(js_context());
}

void register_compressed_texture2d_array() {
	__register_compressed_texture2d_array();
}