...
static JSValue mesh_data_tool_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    MeshDataTool *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<MeshDataTool *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_mesh_data_tool_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_mesh_data_tool_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_mesh_data_tool_enum(ctx, proto);
