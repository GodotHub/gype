...
static void pin_joint2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PinJoint2D"];
...
static JSValue pin_joint2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<PinJoint2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PinJoint2D);
...
static int js_pin_joint2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["PinJoint2D"] = class_id;
	classes_by_id[class_id] = "PinJoint2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &pin_joint2d_class_def);
...
void __register_pin_joint2d() {
	js_init_pin_joint2d_module(js_context());
}

void register_pin_joint2d() {
	__register_pin_joint2d();
}