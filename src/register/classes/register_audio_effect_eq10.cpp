...
static void audio_effect_eq10_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectEQ10"];
...
static JSClassDef audio_effect_eq10_class_def = {
    "AudioEffectEQ10",
    audio_effect_eq10_class_finalizer
};
...
static JSValue audio_effect_eq10_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectEQ10 *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectEQ10 *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectEQ10);
...
static void define_audio_effect_eq10_property(JSContext *ctx, JSValue proto) {
}

static void define_audio_effect_eq10_enum(JSContext *ctx, JSValue proto) {
}

static int js_audio_effect_eq10_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioEffectEQ10"] = class_id;
	classes_by_id[class_id] = "AudioEffectEQ10";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_eq10_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["AudioEffectEQ"]);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, class_id, proto);

	define_audio_effect_eq10_property(ctx, proto);
	define_audio_effect_eq10_enum(ctx, proto);
...
void __register_audio_effect_eq10() {
	js_init_audio_effect_eq10_module(js_context());
}

void register_audio_effect_eq10() {
	__register_audio_effect_eq10();
}