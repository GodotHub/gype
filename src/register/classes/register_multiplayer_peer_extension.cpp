...
static void multiplayer_peer_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["MultiplayerPeerExtension"];
...
static JSClassDef multiplayer_peer_extension_class_def = {
    ...
    multiplayer_peer_extension_class_finalizer
};
...
static JSValue multiplayer_peer_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    MultiplayerPeerExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<MultiplayerPeerExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(MultiplayerPeerExtension);
...
static int js_multiplayer_peer_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["MultiplayerPeerExtension"] = class_id;
	classes_by_id[class_id] = "MultiplayerPeerExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &multiplayer_peer_extension_class_def);
...
void __register_multiplayer_peer_extension() {
	js_init_multiplayer_peer_extension_module(js_context());
}

void register_multiplayer_peer_extension() {
	__register_multiplayer_peer_extension();
}