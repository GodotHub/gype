...
static void visual_shader_node_texture_parameter_triplanar_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeTextureParameterTriplanar"];
...
static JSClassDef visual_shader_node_texture_parameter_triplanar_class_def = {
    ...
    visual_shader_node_texture_parameter_triplanar_class_finalizer
};
...
static JSValue visual_shader_node_texture_parameter_triplanar_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeTextureParameterTriplanar *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeTextureParameterTriplanar *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeTextureParameterTriplanar);
...
static void define_visual_shader_node_texture_parameter_triplanar_enum(JSContext *ctx, JSValue ctor) {
}

static int js_visual_shader_node_texture_parameter_triplanar_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeTextureParameterTriplanar"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeTextureParameterTriplanar";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_texture_parameter_triplanar_class_def);
	...
	define_visual_shader_node_texture_parameter_triplanar_enum(ctx, ctor);
...
void __register_visual_shader_node_texture_parameter_triplanar() {
	js_init_visual_shader_node_texture_parameter_triplanar_module(js_context());
}

void register_visual_shader_node_texture_parameter_triplanar() {
	__register_visual_shader_node_texture_parameter_triplanar();
}