...
static void render_scene_buffers_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RenderSceneBuffersExtension"];
...
static JSClassDef render_scene_buffers_extension_class_def = {
    ...
    render_scene_buffers_extension_class_finalizer
};
...
static JSValue render_scene_buffers_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RenderSceneBuffersExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RenderSceneBuffersExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RenderSceneBuffersExtension);
...
static int js_render_scene_buffers_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["RenderSceneBuffersExtension"] = class_id;
	classes_by_id[class_id] = "RenderSceneBuffersExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &render_scene_buffers_extension_class_def);
...
void __register_render_scene_buffers_extension() {
	js_init_render_scene_buffers_extension_module(js_context());
}

void register_render_scene_buffers_extension() {
	__register_render_scene_buffers_extension();
}