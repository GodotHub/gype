...
static void rd_uniform_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RDUniform"];
...
static JSValue rd_uniform_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<RDUniform *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RDUniform);
...
static void define_rd_uniform_enum(JSContext *ctx, JSValue ctor) {
}

static int js_rd_uniform_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["RDUniform"] = class_id;
	classes_by_id[class_id] = "RDUniform";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &rd_uniform_class_def);
	...
	define_rd_uniform_enum(ctx, ctor);
...
void __register_rd_uniform() {
	js_init_rd_uniform_module(js_context());
}

void register_rd_uniform() {
	__register_rd_uniform();
}