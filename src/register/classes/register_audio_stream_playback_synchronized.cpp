...
static void audio_stream_playback_synchronized_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioStreamPlaybackSynchronized"];
...
static JSClassDef audio_stream_playback_synchronized_class_def = {
    ...
    audio_stream_playback_synchronized_class_finalizer
};
...
static JSValue audio_stream_playback_synchronized_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioStreamPlaybackSynchronized *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioStreamPlaybackSynchronized *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioStreamPlaybackSynchronized);
...
static void define_audio_stream_playback_synchronized_enum(JSContext *ctx, JSValue ctor) {
}

static int js_audio_stream_playback_synchronized_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioStreamPlaybackSynchronized"] = class_id;
	classes_by_id[class_id] = "AudioStreamPlaybackSynchronized";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_stream_playback_synchronized_class_def);
	...
	define_audio_stream_playback_synchronized_enum(ctx, ctor);
...
void __register_audio_stream_playback_synchronized() {
	js_init_audio_stream_playback_synchronized_module(js_context());
}

void register_audio_stream_playback_synchronized() {
	__register_audio_stream_playback_synchronized();
}