...
static void visual_shader_node_face_forward_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeFaceForward"];
...
static JSClassDef visual_shader_node_face_forward_class_def = {
    ...
    visual_shader_node_face_forward_class_finalizer
};
...
static JSValue visual_shader_node_face_forward_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeFaceForward *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeFaceForward *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeFaceForward);
...
static void define_visual_shader_node_face_forward_enum(JSContext *ctx, JSValue ctor) {
}

static int js_visual_shader_node_face_forward_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeFaceForward"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeFaceForward";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_face_forward_class_def);
	...
	define_visual_shader_node_face_forward_enum(ctx, ctor);
...
void __register_visual_shader_node_face_forward() {
	js_init_visual_shader_node_face_forward_module(js_context());
}

void register_visual_shader_node_face_forward() {
	__register_visual_shader_node_face_forward();
}