...
static void audio_stream_player2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioStreamPlayer2D"];
...
static JSClassDef audio_stream_player2d_class_def = {
    "AudioStreamPlayer2D",
    audio_stream_player2d_class_finalizer
};
...
static JSValue audio_stream_player2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioStreamPlayer2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioStreamPlayer2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioStreamPlayer2D);
...
static JSValue audio_stream_player2d_class_set_volume_linear(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioStreamPlayer2D::set_volume_linear, ctx, this_val, argc, argv);
};
static JSValue audio_stream_player2d_class_get_volume_linear(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&AudioStreamPlayer2D::get_volume_linear, ctx, this_val, argc, argv);
}
...
static JSValue audio_stream_player2d_class_set_playing(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioStreamPlayer2D::set_playing, ctx, this_val, argc, argv);
};
...
static const JSCFunctionListEntry audio_stream_player2d_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("set_volume_linear", 1, &audio_stream_player2d_class_set_volume_linear),
	JS_CFUNC_DEF("get_volume_linear", 0, &audio_stream_player2d_class_get_volume_linear),
	...
	JS_CFUNC_DEF("set_playing", 1, &audio_stream_player2d_class_set_playing),
...
static JSValue audio_stream_player2d_class_get_finished_signal(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	AudioStreamPlayer2D *opaque = static_cast<AudioStreamPlayer2D *>(reinterpret_cast<VariantAdapter *>(JS_GetOpaque(this_val, JS_GetClassID(this_val)))->get().operator Object*());
...
static void define_audio_stream_player2d_property(JSContext *ctx, JSValue proto) {
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, audio_stream_player2d_class_get_volume_linear, "get_volume_linear", 0),
        JS_NewCFunction(ctx, audio_stream_player2d_class_set_volume_linear, "set_volume_linear", 1),
        JS_PROP_GETSET
    );
    JS_DefinePropertyGetSet(
        ctx,
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, audio_stream_player2d_class_set_playing, "set_playing", 1),
        JS_PROP_GETSET
    );
    JS_DefinePropertyGetSet(
        ctx,
...
static int js_audio_stream_player2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AudioStreamPlayer2D"] = class_id;
	classes_by_id[class_id] = "AudioStreamPlayer2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_stream_player2d_class_def);
...
void __register_audio_stream_player2d() {
	js_init_audio_stream_player2d_module(js_context());
}

void register_audio_stream_player2d() {
	__register_audio_stream_player2d();
}