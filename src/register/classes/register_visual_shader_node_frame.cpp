...
static void visual_shader_node_frame_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeFrame"];
...
static JSClassDef visual_shader_node_frame_class_def = {
    ...
    visual_shader_node_frame_class_finalizer
};
...
static JSValue visual_shader_node_frame_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeFrame *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeFrame *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeFrame);
...
static JSValue visual_shader_node_frame_class_get_tint_color(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Color {
		VisualShaderNodeFrame *obj = static_cast<VisualShaderNodeFrame *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_tint_color();
	};
	proxy->setter = [this_val](const Color &value) -> void {
		VisualShaderNodeFrame *js_proxy = static_cast<VisualShaderNodeFrame *>(VariantAdapter(this_val).get().operator Object *());
...
static JSValue visual_shader_node_frame_class_get_attached_nodes(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> PackedInt32Array {
		VisualShaderNodeFrame *obj = static_cast<VisualShaderNodeFrame *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_attached_nodes();
	};
	proxy->setter = [this_val](const PackedInt32Array &value) -> void {
		VisualShaderNodeFrame *js_proxy = static_cast<VisualShaderNodeFrame *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_attached_nodes(PackedInt32Array
(Array(value)));
	};
...
static void define_visual_shader_node_frame_enum(JSContext *ctx, JSValue ctor) {
}

static int js_visual_shader_node_frame_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeFrame"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeFrame";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_frame_class_def);
	...
	define_visual_shader_node_frame_enum(ctx, ctor);
...
void __register_visual_shader_node_frame() {
	js_init_visual_shader_node_frame_module(js_context());
}

void register_visual_shader_node_frame() {
	__register_visual_shader_node_frame();
}