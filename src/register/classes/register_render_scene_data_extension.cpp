...
static void render_scene_data_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RenderSceneDataExtension"];
...
static JSClassDef render_scene_data_extension_class_def = {
    ...
    render_scene_data_extension_class_finalizer
};
...
static JSValue render_scene_data_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RenderSceneDataExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RenderSceneDataExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RenderSceneDataExtension);
...
static void define_render_scene_data_extension_property(JSContext *ctx, JSValue proto) {
}

static void define_render_scene_data_extension_enum(JSContext *ctx, JSValue proto) {
}

static int js_render_scene_data_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["RenderSceneDataExtension"] = class_id;
	classes_by_id[class_id] = "RenderSceneDataExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &render_scene_data_extension_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["RenderSceneData"]);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, class_id, proto);

	define_render_scene_data_extension_property(ctx, proto);
	define_render_scene_data_extension_enum(ctx, proto);
...
void __register_render_scene_data_extension() {
	js_init_render_scene_data_extension_module(js_context());
}

void register_render_scene_data_extension() {
	__register_render_scene_data_extension();
}