...
static void animation_root_node_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AnimationRootNode"];
...
static JSClassDef animation_root_node_class_def = {
    "AnimationRootNode",
    animation_root_node_class_finalizer
};
...
static JSValue animation_root_node_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AnimationRootNode *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AnimationRootNode *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AnimationRootNode);
...
static int js_animation_root_node_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AnimationRootNode"] = class_id;
	classes_by_id[class_id] = "AnimationRootNode";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &animation_root_node_class_def);
...
void __register_animation_root_node() {
	js_init_animation_root_node_module(js_context());
}

void register_animation_root_node() {
	__register_animation_root_node();
}