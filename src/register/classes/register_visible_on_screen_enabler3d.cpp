...
static void visible_on_screen_enabler3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisibleOnScreenEnabler3D"];
...
static JSClassDef visible_on_screen_enabler3d_class_def = {
    ...
    visible_on_screen_enabler3d_class_finalizer
};
...
static JSValue visible_on_screen_enabler3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisibleOnScreenEnabler3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisibleOnScreenEnabler3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisibleOnScreenEnabler3D);
...
static JSValue visible_on_screen_enabler3d_class_set_enable_mode(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisibleOnScreenEnabler3D::set_enable_mode, ctx, this_val, argc, argv);
};
...
static JSValue visible_on_screen_enabler3d_class_set_enable_node_path(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&VisibleOnScreenEnabler3D::set_enable_node_path, ctx, this_val, argc, argv);
};
static JSValue visible_on_screen_enabler3d_class_get_enable_node_path(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> NodePath {
		VisibleOnScreenEnabler3D *obj = static_cast<VisibleOnScreenEnabler3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_enable_node_path();
	};
	proxy->setter = [this_val](const NodePath &value) -> void {
		VisibleOnScreenEnabler3D *js_proxy = static_cast<VisibleOnScreenEnabler3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_visible_on_screen_enabler3d_property(JSContext *ctx, JSValue proto) {
...
static int js_visible_on_screen_enabler3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisibleOnScreenEnabler3D"] = class_id;
	classes_by_id[class_id] = "VisibleOnScreenEnabler3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visible_on_screen_enabler3d_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["VisibleOnScreenNotifier3D"]);
...
void __register_visible_on_screen_enabler3d() {
	js_init_visible_on_screen_enabler3d_module(js_context());
}

void register_visible_on_screen_enabler3d() {
	__register_visible_on_screen_enabler3d();
}