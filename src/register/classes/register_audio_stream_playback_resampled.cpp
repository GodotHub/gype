...
static void audio_stream_playback_resampled_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioStreamPlaybackResampled"];
...
static JSClassDef audio_stream_playback_resampled_class_def = {
    ...
    audio_stream_playback_resampled_class_finalizer
};
...
static JSValue audio_stream_playback_resampled_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioStreamPlaybackResampled *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioStreamPlaybackResampled *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioStreamPlaybackResampled);
...
static void define_audio_stream_playback_resampled_enum(JSContext *ctx, JSValue ctor) {
}

static int js_audio_stream_playback_resampled_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioStreamPlaybackResampled"] = class_id;
	classes_by_id[class_id] = "AudioStreamPlaybackResampled";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_stream_playback_resampled_class_def);
	...
	define_audio_stream_playback_resampled_enum(ctx, ctor);
...
void __register_audio_stream_playback_resampled() {
	js_init_audio_stream_playback_resampled_module(js_context());
}

void register_audio_stream_playback_resampled() {
	__register_audio_stream_playback_resampled();
}