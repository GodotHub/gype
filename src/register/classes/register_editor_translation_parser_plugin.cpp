...
static void editor_translation_parser_plugin_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorTranslationParserPlugin"];
...
static JSClassDef editor_translation_parser_plugin_class_def = {
    ...
    editor_translation_parser_plugin_class_finalizer
};
...
static JSValue editor_translation_parser_plugin_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorTranslationParserPlugin *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorTranslationParserPlugin *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorTranslationParserPlugin);
...
static void define_editor_translation_parser_plugin_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_translation_parser_plugin_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorTranslationParserPlugin"] = class_id;
	classes_by_id[class_id] = "EditorTranslationParserPlugin";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_translation_parser_plugin_class_def);
	...
	define_editor_translation_parser_plugin_enum(ctx, ctor);
...
void __register_editor_translation_parser_plugin() {
	js_init_editor_translation_parser_plugin_module(js_context());
}

void register_editor_translation_parser_plugin() {
	__register_editor_translation_parser_plugin();
}