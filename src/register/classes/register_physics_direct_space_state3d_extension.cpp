...
static void physics_direct_space_state3d_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PhysicsDirectSpaceState3DExtension"];
...
static JSClassDef physics_direct_space_state3d_extension_class_def = {
    ...
    physics_direct_space_state3d_extension_class_finalizer
};

static JSValue physics_direct_space_state3d_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    JSClassID class_id = classes["PhysicsDirectSpaceState3DExtension"];
    ...
    PhysicsDirectSpaceState3DExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PhysicsDirectSpaceState3DExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PhysicsDirectSpaceState3DExtension);
...
static void define_physics_direct_space_state3d_extension_enum(JSContext *ctx, JSValue ctor) {
}

static int js_physics_direct_space_state3d_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PhysicsDirectSpaceState3DExtension"] = class_id;
	classes_by_id[class_id] = "PhysicsDirectSpaceState3DExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &physics_direct_space_state3d_extension_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["PhysicsDirectSpaceState3D"]);
	...
	define_physics_direct_space_state3d_extension_enum(ctx, ctor);
...
JSModuleDef *_js_init_physics_direct_space_state3d_extension_module(JSContext *ctx, const char *module_name) {
	...
	if (!m)
		return NULL;
	JS_AddModuleExport(ctx, m, "PhysicsDirectSpaceState3DExtension");
	return m;
}
...
void __register_physics_direct_space_state3d_extension() {
	js_init_physics_direct_space_state3d_extension_module(js_context());
}

void register_physics_direct_space_state3d_extension() {
	__register_physics_direct_space_state3d_extension();
}