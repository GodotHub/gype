...
static void script_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ScriptExtension"];
...
static JSClassDef script_extension_class_def = {
    "ScriptExtension",
    script_extension_class_finalizer
};
...
static JSValue script_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ScriptExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ScriptExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ScriptExtension);
...
static void define_script_extension_property(JSContext *ctx, JSValue proto) {
}

static void define_script_extension_enum(JSContext *ctx, JSValue proto) {
}

static int js_script_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ScriptExtension"] = class_id;
	classes_by_id[class_id] = "ScriptExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &script_extension_class_def);
	...
	define_script_extension_property(ctx, proto);
	define_script_extension_enum(ctx, proto);
...
void __register_script_extension() {
	js_init_script_extension_module(js_context());
}

void register_script_extension() {
	__register_script_extension();
}