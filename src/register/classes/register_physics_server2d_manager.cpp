...
static JSValue physics_server2d_manager_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	JSClassID class_id = classes["_PhysicsServer2DManager"];
	...
	VariantAdapter *adapter = memnew(VariantAdapter(physics_server2d_manager_class));
...
static int js_physics_server2d_manager_class_init(JSContext *ctx, JSModuleDef *m){
	...
	classes["_PhysicsServer2DManager"] = class_id;
	...
	JS_SetModuleExport(ctx, m, "PhysicsServer2DManager", singleton);
...
void register_physics_server2d_manager() {
	_js_init_physics_server2d_manager_module(js_context(), "@godot/classes/physics_server2d_manager");
}