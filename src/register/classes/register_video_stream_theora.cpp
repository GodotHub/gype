...
static void video_stream_theora_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VideoStreamTheora"];
...
static JSClassDef video_stream_theora_class_def = {
    "VideoStreamTheora",
    video_stream_theora_class_finalizer
};
...
static JSValue video_stream_theora_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VideoStreamTheora *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VideoStreamTheora *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VideoStreamTheora);
...
static int js_video_stream_theora_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VideoStreamTheora"] = class_id;
	classes_by_id[class_id] = "VideoStreamTheora";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &video_stream_theora_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["VideoStream"]);
...
void __register_video_stream_theora() {
	js_init_video_stream_theora_module(js_context());
}

void register_video_stream_theora() {
	__register_video_stream_theora();
}