...
static JSValue video_stream_player_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VideoStreamPlayer *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VideoStreamPlayer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue video_stream_player_class_get_bus(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> StringName {
		VideoStreamPlayer *obj = static_cast<VideoStreamPlayer *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_bus();
	};
	proxy->setter = [this_val](const StringName &value) -> void {
		VideoStreamPlayer *js_proxy = static_cast<VideoStreamPlayer *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_video_stream_player_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_video_stream_player_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_video_stream_player_enum(ctx, proto);
