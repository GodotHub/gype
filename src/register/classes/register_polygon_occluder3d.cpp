...
static void polygon_occluder3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PolygonOccluder3D"];
...
static JSClassDef polygon_occluder3d_class_def = {
    "PolygonOccluder3D",
    polygon_occluder3d_class_finalizer
};
...
static JSValue polygon_occluder3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PolygonOccluder3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PolygonOccluder3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PolygonOccluder3D);
...
static JSValue polygon_occluder3d_class_get_polygon(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> PackedVector2Array {
		PolygonOccluder3D *obj = static_cast<PolygonOccluder3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_polygon();
	};
	proxy->setter = [this_val](const PackedVector2Array &value) -> void {
		PolygonOccluder3D *js_proxy = static_cast<PolygonOccluder3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_polygon_occluder3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_polygon_occluder3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PolygonOccluder3D"] = class_id;
	classes_by_id[class_id] = "PolygonOccluder3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &polygon_occluder3d_class_def);
	...
	define_polygon_occluder3d_enum(ctx, ctor);
...
void __register_polygon_occluder3d() {
	js_init_polygon_occluder3d_module(js_context());
}

void register_polygon_occluder3d() {
	__register_polygon_occluder3d();
}