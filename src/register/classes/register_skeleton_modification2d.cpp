...
static void skeleton_modification2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["SkeletonModification2D"];
...
static JSClassDef skeleton_modification2d_class_def = {
    ...
    skeleton_modification2d_class_finalizer
};
...
static JSValue skeleton_modification2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    SkeletonModification2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<SkeletonModification2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(SkeletonModification2D);
...
static void define_skeleton_modification2d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_skeleton_modification2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["SkeletonModification2D"] = class_id;
	classes_by_id[class_id] = "SkeletonModification2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &skeleton_modification2d_class_def);
	...
	define_skeleton_modification2d_enum(ctx, ctor);
...
void __register_skeleton_modification2d() {
	js_init_skeleton_modification2d_module(js_context());
}

void register_skeleton_modification2d() {
	__register_skeleton_modification2d();
}