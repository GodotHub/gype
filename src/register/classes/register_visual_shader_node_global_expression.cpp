...
static void visual_shader_node_global_expression_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeGlobalExpression"];
...
static JSClassDef visual_shader_node_global_expression_class_def = {
    ...
    visual_shader_node_global_expression_class_finalizer
};
...
static JSValue visual_shader_node_global_expression_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeGlobalExpression *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeGlobalExpression *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeGlobalExpression);
...
static void define_visual_shader_node_global_expression_property(JSContext *ctx, JSValue proto) {
}

static void define_visual_shader_node_global_expression_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_global_expression_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeGlobalExpression"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeGlobalExpression";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_global_expression_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["VisualShaderNodeExpression"]);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, class_id, proto);

	define_visual_shader_node_global_expression_property(ctx, proto);
	define_visual_shader_node_global_expression_enum(ctx, proto);
...
void __register_visual_shader_node_global_expression() {
	js_init_visual_shader_node_global_expression_module(js_context());
}

void register_visual_shader_node_global_expression() {
	__register_visual_shader_node_global_expression();
}