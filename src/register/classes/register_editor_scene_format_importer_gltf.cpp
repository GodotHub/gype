...
static void editor_scene_format_importer_gltf_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorSceneFormatImporterGLTF"];
...
static JSClassDef editor_scene_format_importer_gltf_class_def = {
    ...
    editor_scene_format_importer_gltf_class_finalizer
};
...
static JSValue editor_scene_format_importer_gltf_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorSceneFormatImporterGLTF *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorSceneFormatImporterGLTF *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorSceneFormatImporterGLTF);
...
static void define_editor_scene_format_importer_gltf_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_scene_format_importer_gltf_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorSceneFormatImporterGLTF"] = class_id;
	classes_by_id[class_id] = "EditorSceneFormatImporterGLTF";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_scene_format_importer_gltf_class_def);
	...
	define_editor_scene_format_importer_gltf_enum(ctx, ctor);
...
void __register_editor_scene_format_importer_gltf() {
	js_init_editor_scene_format_importer_gltf_module(js_context());
}

void register_editor_scene_format_importer_gltf() {
	__register_editor_scene_format_importer_gltf();
}