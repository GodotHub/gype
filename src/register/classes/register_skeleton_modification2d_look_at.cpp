...
static JSValue skeleton_modification2d_look_at_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    SkeletonModification2DLookAt *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<SkeletonModification2DLookAt *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue skeleton_modification2d_look_at_class_get_bone2d_node(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> NodePath {
		SkeletonModification2DLookAt *obj = static_cast<SkeletonModification2DLookAt *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_bone2d_node();
	};
	proxy->setter = [this_val](const NodePath &value) -> void {
		SkeletonModification2DLookAt *js_proxy = static_cast<SkeletonModification2DLookAt *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_bone2d_node(value);
	};
