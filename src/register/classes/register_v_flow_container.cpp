...
static void v_flow_container_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VFlowContainer"];
...
static JSClassDef v_flow_container_class_def = {
    "VFlowContainer",
    v_flow_container_class_finalizer
};
...
static JSValue v_flow_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VFlowContainer *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VFlowContainer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VFlowContainer);
...
static void define_v_flow_container_enum(JSContext *ctx, JSValue ctor) {
}

static int js_v_flow_container_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VFlowContainer"] = class_id;
	classes_by_id[class_id] = "VFlowContainer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &v_flow_container_class_def);
	...
	define_v_flow_container_enum(ctx, ctor);
...
void __register_v_flow_container() {
	js_init_v_flow_container_module(js_context());
}

void register_v_flow_container() {
	__register_v_flow_container();
}