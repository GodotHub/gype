...
static void visual_shader_node_switch_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeSwitch"];
...
static JSClassDef visual_shader_node_switch_class_def = {
    ...
    visual_shader_node_switch_class_finalizer
};
...
static JSValue visual_shader_node_switch_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeSwitch *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeSwitch *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeSwitch);
...
static JSValue visual_shader_node_switch_class_set_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeSwitch::set_op_type, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_switch_class_get_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeSwitch::get_op_type, ctx, this_val, argc, argv);
}
...
static void define_visual_shader_node_switch_property(JSContext *ctx, JSValue proto) {
...
static int js_visual_shader_node_switch_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeSwitch"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeSwitch";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_switch_class_def);
...
void __register_visual_shader_node_switch() {
	js_init_visual_shader_node_switch_module(js_context());
}

void register_visual_shader_node_switch() {
	__register_visual_shader_node_switch();
}