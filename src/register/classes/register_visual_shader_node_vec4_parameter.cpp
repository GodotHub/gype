...
static void visual_shader_node_vec4_parameter_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeVec4Parameter"];
...
static JSClassDef visual_shader_node_vec4_parameter_class_def = {
    ...
    visual_shader_node_vec4_parameter_class_finalizer
};
...
static JSValue visual_shader_node_vec4_parameter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeVec4Parameter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeVec4Parameter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeVec4Parameter);
...
static JSValue visual_shader_node_vec4_parameter_class_is_default_value_enabled(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeVec4Parameter::is_default_value_enabled, ctx, this_val, argc, argv);
}
...
static JSValue visual_shader_node_vec4_parameter_class_get_default_value(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	ObjectProxy<Vector4> *proxy = memnew(ObjectProxy<Vector4>);
	proxy->wrapped = VariantAdapter(this_val).get();
	proxy->getter = [this_val]() -> Vector4 {
		VisualShaderNodeVec4Parameter *obj = static_cast<VisualShaderNodeVec4Parameter *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_default_value();
	};
	proxy->setter = [this_val](const Vector4 &value) -> void {
		VisualShaderNodeVec4Parameter *js_proxy = static_cast<VisualShaderNodeVec4Parameter *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_default_value(value);
	};
	JSValue obj = JS_NewObjectClass(ctx, classes["Vector4Proxy"]);
	if (is_exception(ctx, obj)) {
		return JS_EXCEPTION;
	}
	JS_SetOpaque(obj, &proxy);
	JSValue global = JS_GetGlobalObject(ctx);
	JSValue obj_constructor = JS_GetPropertyStr(ctx, global, "Vector4Proxy");
...
static void define_visual_shader_node_vec4_parameter_enum(JSContext *ctx, JSValue ctor) {
}

static int js_visual_shader_node_vec4_parameter_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeVec4Parameter"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeVec4Parameter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_vec4_parameter_class_def);
	...
	define_visual_shader_node_vec4_parameter_enum(ctx, ctor);
...
void __register_visual_shader_node_vec4_parameter() {
	js_init_visual_shader_node_vec4_parameter_module(js_context());
}

void register_visual_shader_node_vec4_parameter() {
	__register_visual_shader_node_vec4_parameter();
}