...
static void shader_globals_override_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ShaderGlobalsOverride"];
...
static JSClassDef shader_globals_override_class_def = {
    ...
    shader_globals_override_class_finalizer
};
...
static JSValue shader_globals_override_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ShaderGlobalsOverride *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ShaderGlobalsOverride *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ShaderGlobalsOverride);
...
static void define_shader_globals_override_enum(JSContext *ctx, JSValue ctor) {
}

static int js_shader_globals_override_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ShaderGlobalsOverride"] = class_id;
	classes_by_id[class_id] = "ShaderGlobalsOverride";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &shader_globals_override_class_def);
	...
	define_shader_globals_override_enum(ctx, ctor);
...
void __register_shader_globals_override() {
	js_init_shader_globals_override_module(js_context());
}

void register_shader_globals_override() {
	__register_shader_globals_override();
}