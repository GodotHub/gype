...
static void popup_panel_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PopupPanel"];
...
static JSValue popup_panel_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<PopupPanel *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PopupPanel);
...
static void define_popup_panel_enum(JSContext *ctx, JSValue ctor) {
}

static int js_popup_panel_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PopupPanel"] = class_id;
	classes_by_id[class_id] = "PopupPanel";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &popup_panel_class_def);
	...
	define_popup_panel_enum(ctx, ctor);
...
void __register_popup_panel() {
	js_init_popup_panel_module(js_context());
}

void register_popup_panel() {
	__register_popup_panel();
}