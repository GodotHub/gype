...
static void render_data_rd_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RenderDataRD"];
...
static JSClassDef render_data_rd_class_def = {
    "RenderDataRD",
    render_data_rd_class_finalizer
};
...
static JSValue render_data_rd_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RenderDataRD *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RenderDataRD *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RenderDataRD);
...
static void define_render_data_rd_enum(JSContext *ctx, JSValue ctor) {
}

static int js_render_data_rd_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["RenderDataRD"] = class_id;
	classes_by_id[class_id] = "RenderDataRD";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &render_data_rd_class_def);
	...
	define_render_data_rd_enum(ctx, ctor);
...
void __register_render_data_rd() {
	js_init_render_data_rd_module(js_context());
}

void register_render_data_rd() {
	__register_render_data_rd();
}