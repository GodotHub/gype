...
static void character_body2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CharacterBody2D"];
...
static JSClassDef character_body2d_class_def = {
    "CharacterBody2D",
    character_body2d_class_finalizer
};
...
static JSValue character_body2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<CharacterBody2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CharacterBody2D);
...
static JSValue character_body2d_class_get_velocity(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		CharacterBody2D *obj = static_cast<CharacterBody2D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_velocity();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		CharacterBody2D *js_proxy = static_cast<CharacterBody2D *>(VariantAdapter(this_val).get().operator Object *());
...
static int js_character_body2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["CharacterBody2D"] = class_id;
	classes_by_id[class_id] = "CharacterBody2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &character_body2d_class_def);
...
void __register_character_body2d() {
	js_init_character_body2d_module(js_context());
}

void register_character_body2d() {
	__register_character_body2d();
}