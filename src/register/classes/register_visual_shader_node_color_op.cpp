...
static void visual_shader_node_color_op_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeColorOp"];
...
static JSClassDef visual_shader_node_color_op_class_def = {
    ...
    visual_shader_node_color_op_class_finalizer
};
...
static JSValue visual_shader_node_color_op_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeColorOp *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeColorOp *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeColorOp);
...
static JSValue visual_shader_node_color_op_class_set_operator(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeColorOp::set_operator, ctx, this_val, argc, argv);
};
...
static void define_visual_shader_node_color_op_property(JSContext *ctx, JSValue proto) {
...
static void define_visual_shader_node_color_op_enum(JSContext *ctx, JSValue proto) {
...
static int js_visual_shader_node_color_op_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeColorOp"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeColorOp";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_color_op_class_def);
	...
	define_visual_shader_node_color_op_enum(ctx, proto);
...
void __register_visual_shader_node_color_op() {
	js_init_visual_shader_node_color_op_module(js_context());
}

void register_visual_shader_node_color_op() {
	__register_visual_shader_node_color_op();
}