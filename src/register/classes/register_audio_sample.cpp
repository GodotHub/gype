...
static void audio_sample_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioSample"];
...
static JSValue audio_sample_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<AudioSample *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioSample);
...
static void define_audio_sample_property(JSContext *ctx, JSValue proto) {
}

static void define_audio_sample_enum(JSContext *ctx, JSValue proto) {
}

static int js_audio_sample_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioSample"] = class_id;
	classes_by_id[class_id] = "AudioSample";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_sample_class_def);
	...
	define_audio_sample_enum(ctx, proto);
...
void __register_audio_sample() {
	js_init_audio_sample_module(js_context());
}

void register_audio_sample() {
	__register_audio_sample();
}