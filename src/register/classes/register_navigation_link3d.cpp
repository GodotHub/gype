...
static void navigation_link3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["NavigationLink3D"];
...
static JSClassDef navigation_link3d_class_def = {
    "NavigationLink3D",
    navigation_link3d_class_finalizer
};
...
static JSValue navigation_link3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    NavigationLink3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<NavigationLink3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(NavigationLink3D);
...
static JSValue navigation_link3d_class_set_navigation_map(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&NavigationLink3D::set_navigation_map, ctx, this_val, argc, argv);
};
static JSValue navigation_link3d_class_get_navigation_map(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&NavigationLink3D::get_navigation_map, ctx, this_val, argc, argv);
};
...
static JSValue navigation_link3d_class_get_start_position(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		NavigationLink3D *obj = static_cast<NavigationLink3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_start_position();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		NavigationLink3D *js_proxy = static_cast<NavigationLink3D *>(VariantAdapter(this_val).get().operator Object *());
...
static const JSCFunctionListEntry navigation_link3d_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("set_navigation_map", 1, &navigation_link3d_class_set_navigation_map),
	JS_CFUNC_DEF("get_navigation_map", 0, &navigation_link3d_class_get_navigation_map),
...
static void define_navigation_link3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_navigation_link3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["NavigationLink3D"] = class_id;
	classes_by_id[class_id] = "NavigationLink3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &navigation_link3d_class_def);
	...
	define_navigation_link3d_enum(ctx, ctor);
...
void __register_navigation_link3d() {
	js_init_navigation_link3d_module(js_context());
}

void register_navigation_link3d() {
	__register_navigation_link3d();
}