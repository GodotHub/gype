...
static void xr_controller_tracker_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["XRControllerTracker"];
...
static JSClassDef xr_controller_tracker_class_def = {
    "XRControllerTracker",
    xr_controller_tracker_class_finalizer
};
...
static JSValue xr_controller_tracker_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    XRControllerTracker *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<XRControllerTracker *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(XRControllerTracker);
...
static void define_xr_controller_tracker_enum(JSContext *ctx, JSValue ctor) {
}

static int js_xr_controller_tracker_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["XRControllerTracker"] = class_id;
	classes_by_id[class_id] = "XRControllerTracker";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &xr_controller_tracker_class_def);
	...
	define_xr_controller_tracker_enum(ctx, ctor);
...
void __register_xr_controller_tracker() {
	js_init_xr_controller_tracker_module(js_context());
}

void register_xr_controller_tracker() {
	__register_xr_controller_tracker();
}