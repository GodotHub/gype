...
static void visual_shader_node_particle_accelerator_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeParticleAccelerator"];
...
static JSClassDef visual_shader_node_particle_accelerator_class_def = {
    ...
    visual_shader_node_particle_accelerator_class_finalizer
};
...
static JSValue visual_shader_node_particle_accelerator_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeParticleAccelerator *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeParticleAccelerator *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeParticleAccelerator);
...
static void define_visual_shader_node_particle_accelerator_enum(JSContext *ctx, JSValue ctor) {
...
static int js_visual_shader_node_particle_accelerator_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeParticleAccelerator"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeParticleAccelerator";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_particle_accelerator_class_def);
	...
	define_visual_shader_node_particle_accelerator_enum(ctx, ctor);
...
void __register_visual_shader_node_particle_accelerator() {
	js_init_visual_shader_node_particle_accelerator_module(js_context());
}

void register_visual_shader_node_particle_accelerator() {
	__register_visual_shader_node_particle_accelerator();
}