...
static void visual_shader_node_if_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeIf"];
...
static JSClassDef visual_shader_node_if_class_def = {
    "VisualShaderNodeIf",
    visual_shader_node_if_class_finalizer
};
...
static JSValue visual_shader_node_if_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeIf *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeIf *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeIf);
...
static void define_visual_shader_node_if_property(JSContext *ctx, JSValue proto) {
}

static void define_visual_shader_node_if_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_if_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeIf"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeIf";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_if_class_def);
	...
	define_visual_shader_node_if_enum(ctx, proto);
...
void __register_visual_shader_node_if() {
	js_init_visual_shader_node_if_module(js_context());
}

void register_visual_shader_node_if() {
	__register_visual_shader_node_if();
}