...
static JSValue rd_sampler_state_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RDSamplerState *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RDSamplerState *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_rd_sampler_state_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_rd_sampler_state_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_rd_sampler_state_enum(ctx, proto);
