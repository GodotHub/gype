...
static void visual_shader_node_texture3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeTexture3D"];
...
static JSClassDef visual_shader_node_texture3d_class_def = {
    ...
    visual_shader_node_texture3d_class_finalizer
};
...
static JSValue visual_shader_node_texture3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeTexture3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeTexture3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeTexture3D);
...
static void define_visual_shader_node_texture3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_visual_shader_node_texture3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeTexture3D"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeTexture3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_texture3d_class_def);
	...
	define_visual_shader_node_texture3d_enum(ctx, ctor);
...
void __register_visual_shader_node_texture3d() {
	js_init_visual_shader_node_texture3d_module(js_context());
}

void register_visual_shader_node_texture3d() {
	__register_visual_shader_node_texture3d();
}