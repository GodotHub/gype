...
static void open_xr_composition_layer_equirect_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["OpenXRCompositionLayerEquirect"];
...
static JSClassDef open_xr_composition_layer_equirect_class_def = {
    ...
    open_xr_composition_layer_equirect_class_finalizer
};
...
static JSValue open_xr_composition_layer_equirect_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    OpenXRCompositionLayerEquirect *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<OpenXRCompositionLayerEquirect *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(OpenXRCompositionLayerEquirect);
...
static void define_open_xr_composition_layer_equirect_enum(JSContext *ctx, JSValue ctor) {
}

static int js_open_xr_composition_layer_equirect_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["OpenXRCompositionLayerEquirect"] = class_id;
	classes_by_id[class_id] = "OpenXRCompositionLayerEquirect";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &open_xr_composition_layer_equirect_class_def);
	...
	define_open_xr_composition_layer_equirect_enum(ctx, ctor);
...
void __register_open_xr_composition_layer_equirect() {
	js_init_open_xr_composition_layer_equirect_module(js_context());
}

void register_open_xr_composition_layer_equirect() {
	__register_open_xr_composition_layer_equirect();
}