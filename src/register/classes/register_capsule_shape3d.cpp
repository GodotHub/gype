...
static void capsule_shape3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CapsuleShape3D"];
...
static JSClassDef capsule_shape3d_class_def = {
    "CapsuleShape3D",
    capsule_shape3d_class_finalizer
};
...
static JSValue capsule_shape3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CapsuleShape3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CapsuleShape3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CapsuleShape3D);
...
static JSValue capsule_shape3d_class_set_mid_height(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&CapsuleShape3D::set_mid_height, ctx, this_val, argc, argv);
};
static JSValue capsule_shape3d_class_get_mid_height(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&CapsuleShape3D::get_mid_height, ctx, this_val, argc, argv);
}
...
static const JSCFunctionListEntry capsule_shape3d_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("set_mid_height", 1, &capsule_shape3d_class_set_mid_height),
	JS_CFUNC_DEF("get_mid_height", 0, &capsule_shape3d_class_get_mid_height),
};
...
static void define_capsule_shape3d_property(JSContext *ctx, JSValue proto) {
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, capsule_shape3d_class_get_mid_height, "get_mid_height", 0),
        JS_NewCFunction(ctx, capsule_shape3d_class_set_mid_height, "set_mid_height", 1),
        JS_PROP_GETSET
    );
}

static void define_capsule_shape3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_capsule_shape3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CapsuleShape3D"] = class_id;
	classes_by_id[class_id] = "CapsuleShape3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &capsule_shape3d_class_def);
	...
	define_capsule_shape3d_enum(ctx, ctor);
...
void __register_capsule_shape3d() {
	js_init_capsule_shape3d_module(js_context());
}

void register_capsule_shape3d() {
	__register_capsule_shape3d();
}