...
static JSValue performance_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	JSClassID class_id = classes["_Performance"];
	...
	VariantAdapter *adapter = memnew(VariantAdapter(performance_class));
...
static int js_performance_class_init(JSContext *ctx, JSModuleDef *m){
	...
	JS_SetModuleExport(ctx, m, "Performance", singleton);
...
void register_performance() {
	_js_init_performance_module(js_context(), "@godot/classes/performance");
}