...
static void placeholder_texture3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PlaceholderTexture3D"];
...
static JSClassDef placeholder_texture3d_class_def = {
    ...
    placeholder_texture3d_class_finalizer
};
...
static JSValue placeholder_texture3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PlaceholderTexture3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PlaceholderTexture3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PlaceholderTexture3D);
...
static JSValue placeholder_texture3d_class_set_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&PlaceholderTexture3D::set_size, ctx, this_val, argc, argv);
};
static JSValue placeholder_texture3d_class_get_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	ObjectProxy<Vector3i> *proxy = memnew(ObjectProxy<Vector3i>);
	proxy->wrapped = VariantAdapter(this_val).get();
	proxy->getter = [this_val]() -> Vector3i {
		PlaceholderTexture3D *obj = static_cast<PlaceholderTexture3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_size();
	};
	proxy->setter = [this_val](const Vector3i &value) -> void {
		PlaceholderTexture3D *js_proxy = static_cast<PlaceholderTexture3D *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_size(value);
	};
	JSValue obj = JS_NewObjectClass(ctx, classes["Vector3iProxy"]);
	if (is_exception(ctx, obj)) {
		return JS_EXCEPTION;
	}
	JS_SetOpaque(obj, &proxy);
	JSValue global = JS_GetGlobalObject(ctx);
	JSValue obj_constructor = JS_GetPropertyStr(ctx, global, "Vector3iProxy");
...
static void define_placeholder_texture3d_property(JSContext *ctx, JSValue proto) {
...
static void define_placeholder_texture3d_enum(JSContext *ctx, JSValue proto) {
}

static int js_placeholder_texture3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PlaceholderTexture3D"] = class_id;
	classes_by_id[class_id] = "PlaceholderTexture3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &placeholder_texture3d_class_def);
	...
	define_placeholder_texture3d_enum(ctx, proto);
...
void __register_placeholder_texture3d() {
	js_init_placeholder_texture3d_module(js_context());
}

void register_placeholder_texture3d() {
	__register_placeholder_texture3d();
}