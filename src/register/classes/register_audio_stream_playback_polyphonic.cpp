...
static void audio_stream_playback_polyphonic_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioStreamPlaybackPolyphonic"];
...
static JSClassDef audio_stream_playback_polyphonic_class_def = {
    ...
    audio_stream_playback_polyphonic_class_finalizer
};
...
static JSValue audio_stream_playback_polyphonic_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioStreamPlaybackPolyphonic *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioStreamPlaybackPolyphonic *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioStreamPlaybackPolyphonic);
...
static int js_audio_stream_playback_polyphonic_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AudioStreamPlaybackPolyphonic"] = class_id;
	classes_by_id[class_id] = "AudioStreamPlaybackPolyphonic";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_stream_playback_polyphonic_class_def);
...
void __register_audio_stream_playback_polyphonic() {
	js_init_audio_stream_playback_polyphonic_module(js_context());
}

void register_audio_stream_playback_polyphonic() {
	__register_audio_stream_playback_polyphonic();
}