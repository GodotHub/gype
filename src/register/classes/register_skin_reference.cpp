...
static void skin_reference_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["SkinReference"];
...
static JSClassDef skin_reference_class_def = {
    "SkinReference",
    skin_reference_class_finalizer
};
...
static JSValue skin_reference_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    SkinReference *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<SkinReference *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(SkinReference);
...
static int js_skin_reference_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["SkinReference"] = class_id;
	classes_by_id[class_id] = "SkinReference";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &skin_reference_class_def);
...
void __register_skin_reference() {
	js_init_skin_reference_module(js_context());
}

void register_skin_reference() {
	__register_skin_reference();
}