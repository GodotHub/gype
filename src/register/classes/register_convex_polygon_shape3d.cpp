...
static void convex_polygon_shape3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ConvexPolygonShape3D"];
...
static JSClassDef convex_polygon_shape3d_class_def = {
    ...
    convex_polygon_shape3d_class_finalizer
};
...
static JSValue convex_polygon_shape3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ConvexPolygonShape3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ConvexPolygonShape3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ConvexPolygonShape3D);
...
static JSValue convex_polygon_shape3d_class_get_points(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> PackedVector3Array {
		ConvexPolygonShape3D *obj = static_cast<ConvexPolygonShape3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_points();
	};
	proxy->setter = [this_val](const PackedVector3Array &value) -> void {
		ConvexPolygonShape3D *js_proxy = static_cast<ConvexPolygonShape3D *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_points(PackedVector3Array
(Array(value)));
	};
...
static int js_convex_polygon_shape3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["ConvexPolygonShape3D"] = class_id;
	classes_by_id[class_id] = "ConvexPolygonShape3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &convex_polygon_shape3d_class_def);
...
void __register_convex_polygon_shape3d() {
	js_init_convex_polygon_shape3d_module(js_context());
}

void register_convex_polygon_shape3d() {
	__register_convex_polygon_shape3d();
}