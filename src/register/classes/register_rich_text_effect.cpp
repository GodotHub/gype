...
static void rich_text_effect_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RichTextEffect"];
...
static JSClassDef rich_text_effect_class_def = {
    "RichTextEffect",
    rich_text_effect_class_finalizer
};
...
static JSValue rich_text_effect_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RichTextEffect *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RichTextEffect *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RichTextEffect);
...
static void define_rich_text_effect_enum(JSContext *ctx, JSValue ctor) {
}

static int js_rich_text_effect_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["RichTextEffect"] = class_id;
	classes_by_id[class_id] = "RichTextEffect";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &rich_text_effect_class_def);
	...
	define_rich_text_effect_enum(ctx, ctor);
...
void __register_rich_text_effect() {
	js_init_rich_text_effect_module(js_context());
}

void register_rich_text_effect() {
	__register_rich_text_effect();
}