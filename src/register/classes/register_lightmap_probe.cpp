...
static void lightmap_probe_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["LightmapProbe"];
...
static JSClassDef lightmap_probe_class_def = {
    "LightmapProbe",
    lightmap_probe_class_finalizer
};
...
static JSValue lightmap_probe_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    LightmapProbe *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<LightmapProbe *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(LightmapProbe);
...
static void define_lightmap_probe_enum(JSContext *ctx, JSValue ctor) {
}

static int js_lightmap_probe_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["LightmapProbe"] = class_id;
	classes_by_id[class_id] = "LightmapProbe";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &lightmap_probe_class_def);
	...
	define_lightmap_probe_enum(ctx, ctor);
...
void __register_lightmap_probe() {
	js_init_lightmap_probe_module(js_context());
}

void register_lightmap_probe() {
	__register_lightmap_probe();
}