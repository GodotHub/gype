...
static void input_event_screen_touch_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["InputEventScreenTouch"];
...
static JSClassDef input_event_screen_touch_class_def = {
    ...
    input_event_screen_touch_class_finalizer
};
...
static JSValue input_event_screen_touch_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    InputEventScreenTouch *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<InputEventScreenTouch *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(InputEventScreenTouch);
...
static JSValue input_event_screen_touch_class_get_position(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		InputEventScreenTouch *obj = static_cast<InputEventScreenTouch *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_position();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		InputEventScreenTouch *js_proxy = static_cast<InputEventScreenTouch *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_input_event_screen_touch_enum(JSContext *ctx, JSValue ctor) {
}

static int js_input_event_screen_touch_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["InputEventScreenTouch"] = class_id;
	classes_by_id[class_id] = "InputEventScreenTouch";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &input_event_screen_touch_class_def);
	...
	define_input_event_screen_touch_enum(ctx, ctor);
...
void __register_input_event_screen_touch() {
	js_init_input_event_screen_touch_module(js_context());
}

void register_input_event_screen_touch() {
	__register_input_event_screen_touch();
}