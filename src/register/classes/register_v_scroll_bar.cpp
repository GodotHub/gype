...
static void v_scroll_bar_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VScrollBar"];
...
static JSValue v_scroll_bar_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<VScrollBar *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VScrollBar);
...
static int js_v_scroll_bar_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VScrollBar"] = class_id;
	classes_by_id[class_id] = "VScrollBar";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &v_scroll_bar_class_def);
...
void __register_v_scroll_bar() {
	js_init_v_scroll_bar_module(js_context());
}

void register_v_scroll_bar() {
	__register_v_scroll_bar();
}