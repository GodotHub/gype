...
static void compressed_texture3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CompressedTexture3D"];
...
static JSClassDef compressed_texture3d_class_def = {
    "CompressedTexture3D",
    compressed_texture3d_class_finalizer
};
...
static JSValue compressed_texture3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CompressedTexture3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CompressedTexture3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CompressedTexture3D);
...
static void define_compressed_texture3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_compressed_texture3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CompressedTexture3D"] = class_id;
	classes_by_id[class_id] = "CompressedTexture3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &compressed_texture3d_class_def);
	...
	define_compressed_texture3d_enum(ctx, ctor);
...
void __register_compressed_texture3d() {
	js_init_compressed_texture3d_module(js_context());
}

void register_compressed_texture3d() {
	__register_compressed_texture3d();
}