...
static void orm_material3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ORMMaterial3D"];
...
static JSClassDef orm_material3d_class_def = {
    "ORMMaterial3D",
    orm_material3d_class_finalizer
};
...
static JSValue orm_material3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ORMMaterial3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ORMMaterial3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ORMMaterial3D);
...
static void define_orm_material3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_orm_material3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ORMMaterial3D"] = class_id;
	classes_by_id[class_id] = "ORMMaterial3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &orm_material3d_class_def);
	...
	define_orm_material3d_enum(ctx, ctor);
...
void __register_orm_material3d() {
	js_init_orm_material3d_module(js_context());
}

void register_orm_material3d() {
	__register_orm_material3d();
}