...
static JSValue physics_test_motion_parameters2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PhysicsTestMotionParameters2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PhysicsTestMotionParameters2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_physics_test_motion_parameters2d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_physics_test_motion_parameters2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_physics_test_motion_parameters2d_enum(ctx, proto);
