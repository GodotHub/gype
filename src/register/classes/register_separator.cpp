...
static void separator_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Separator"];
...
static JSValue separator_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Separator *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Separator);
...
static void define_separator_enum(JSContext *ctx, JSValue ctor) {
}

static int js_separator_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Separator"] = class_id;
	classes_by_id[class_id] = "Separator";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &separator_class_def);
	...
	define_separator_enum(ctx, ctor);
...
void __register_separator() {
	js_init_separator_module(js_context());
}

void register_separator() {
	__register_separator();
}