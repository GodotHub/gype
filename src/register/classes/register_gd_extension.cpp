...
static JSValue gd_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    GDExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<GDExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_gd_extension_enum(JSContext *ctx, JSValue proto) {
	...
	JS_SetPropertyStr(ctx, proto, "InitializationLevel", InitializationLevel_obj);
}
