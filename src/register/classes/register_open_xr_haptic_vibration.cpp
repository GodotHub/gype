...
static JSValue open_xr_haptic_vibration_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    OpenXRHapticVibration *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<OpenXRHapticVibration *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_open_xr_haptic_vibration_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_open_xr_haptic_vibration_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_open_xr_haptic_vibration_enum(ctx, proto);
