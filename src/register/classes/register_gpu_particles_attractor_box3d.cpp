...
static void gpu_particles_attractor_box3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["GPUParticlesAttractorBox3D"];
...
static JSClassDef gpu_particles_attractor_box3d_class_def = {
    ...
    gpu_particles_attractor_box3d_class_finalizer
};
...
static JSValue gpu_particles_attractor_box3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    GPUParticlesAttractorBox3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<GPUParticlesAttractorBox3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(GPUParticlesAttractorBox3D);
...
static JSValue gpu_particles_attractor_box3d_class_get_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		GPUParticlesAttractorBox3D *obj = static_cast<GPUParticlesAttractorBox3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_size();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		GPUParticlesAttractorBox3D *js_proxy = static_cast<GPUParticlesAttractorBox3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_gpu_particles_attractor_box3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_gpu_particles_attractor_box3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["GPUParticlesAttractorBox3D"] = class_id;
	classes_by_id[class_id] = "GPUParticlesAttractorBox3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &gpu_particles_attractor_box3d_class_def);
	...
	define_gpu_particles_attractor_box3d_enum(ctx, ctor);
...
void __register_gpu_particles_attractor_box3d() {
	js_init_gpu_particles_attractor_box3d_module(js_context());
}

void register_gpu_particles_attractor_box3d() {
	__register_gpu_particles_attractor_box3d();
}