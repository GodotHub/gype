...
static void visual_shader_node_u_int_func_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeUIntFunc"];
...
static JSClassDef visual_shader_node_u_int_func_class_def = {
    ...
    visual_shader_node_u_int_func_class_finalizer
};
...
static JSValue visual_shader_node_u_int_func_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeUIntFunc *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeUIntFunc *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeUIntFunc);
...
static JSValue visual_shader_node_u_int_func_class_set_function(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeUIntFunc::set_function, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_u_int_func_class_get_function(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeUIntFunc::get_function, ctx, this_val, argc, argv);
}
...
static void define_visual_shader_node_u_int_func_property(JSContext *ctx, JSValue proto) {
    JS_DefinePropertyGetSet(
        ctx,
        proto,
        JS_NewAtom(ctx, "function"),
        JS_NewCFunction(ctx, visual_shader_node_u_int_func_class_get_function, "get_function", 0),
        JS_NewCFunction(ctx, visual_shader_node_u_int_func_class_set_function, "set_function", 1),
        JS_PROP_GETSET
    );
}

static void define_visual_shader_node_u_int_func_enum(JSContext *ctx, JSValue proto) {
	JSValue Function_obj = JS_NewObject(ctx);
	JS_SetPropertyStr(ctx, Function_obj, "FUNC_NEGATE", JS_NewInt64(ctx, 0));
	JS_SetPropertyStr(ctx, Function_obj, "FUNC_BITWISE_NOT", JS_NewInt64(ctx, 1));
...
static int js_visual_shader_node_u_int_func_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeUIntFunc"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeUIntFunc";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_u_int_func_class_def);
	...
	define_visual_shader_node_u_int_func_property(ctx, proto);
	define_visual_shader_node_u_int_func_enum(ctx, proto);
...
void __register_visual_shader_node_u_int_func() {
	js_init_visual_shader_node_u_int_func_module(js_context());
}

void register_visual_shader_node_u_int_func() {
	__register_visual_shader_node_u_int_func();
}