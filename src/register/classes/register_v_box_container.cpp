...
static void v_box_container_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VBoxContainer"];
...
static JSClassDef v_box_container_class_def = {
    "VBoxContainer",
    v_box_container_class_finalizer
};
...
static JSValue v_box_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<VBoxContainer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VBoxContainer);
...
static int js_v_box_container_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VBoxContainer"] = class_id;
	classes_by_id[class_id] = "VBoxContainer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &v_box_container_class_def);
...
void __register_v_box_container() {
	js_init_v_box_container_module(js_context());
}

void register_v_box_container() {
	__register_v_box_container();
}