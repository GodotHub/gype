...
static void circle_shape2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CircleShape2D"];
...
static JSClassDef circle_shape2d_class_def = {
    "CircleShape2D",
    circle_shape2d_class_finalizer
};
...
static JSValue circle_shape2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CircleShape2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CircleShape2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CircleShape2D);
...
static JSValue circle_shape2d_class_set_radius(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&CircleShape2D::set_radius, ctx, this_val, argc, argv);
};
...
static void define_circle_shape2d_property(JSContext *ctx, JSValue proto) {
...
static void define_circle_shape2d_enum(JSContext *ctx, JSValue proto) {
}

static int js_circle_shape2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CircleShape2D"] = class_id;
	classes_by_id[class_id] = "CircleShape2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &circle_shape2d_class_def);
	...
	define_circle_shape2d_enum(ctx, proto);
...
void __register_circle_shape2d() {
	js_init_circle_shape2d_module(js_context());
}

void register_circle_shape2d() {
	__register_circle_shape2d();
}