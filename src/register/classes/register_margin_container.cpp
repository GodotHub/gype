...
static void margin_container_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["MarginContainer"];
...
static JSClassDef margin_container_class_def = {
    "MarginContainer",
    margin_container_class_finalizer
};
...
static JSValue margin_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<MarginContainer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(MarginContainer);
...
static void define_margin_container_enum(JSContext *ctx, JSValue ctor) {
}

static int js_margin_container_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["MarginContainer"] = class_id;
	classes_by_id[class_id] = "MarginContainer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &margin_container_class_def);
	...
	define_margin_container_enum(ctx, ctor);
...
void __register_margin_container() {
	js_init_margin_container_module(js_context());
}

void register_margin_container() {
	__register_margin_container();
}