...
static void compressed_cubemap_array_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CompressedCubemapArray"];
...
static JSClassDef compressed_cubemap_array_class_def = {
    ...
    compressed_cubemap_array_class_finalizer
};
...
static JSValue compressed_cubemap_array_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    CompressedCubemapArray *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<CompressedCubemapArray *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CompressedCubemapArray);
...
static void define_compressed_cubemap_array_enum(JSContext *ctx, JSValue ctor) {
}

static int js_compressed_cubemap_array_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CompressedCubemapArray"] = class_id;
	classes_by_id[class_id] = "CompressedCubemapArray";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &compressed_cubemap_array_class_def);
	...
	define_compressed_cubemap_array_enum(ctx, ctor);
...
void __register_compressed_cubemap_array() {
	js_init_compressed_cubemap_array_module(js_context());
}

void register_compressed_cubemap_array() {
	__register_compressed_cubemap_array();
}