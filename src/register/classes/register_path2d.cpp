...
static void path2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Path2D"];
...
static JSValue path2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Path2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Path2D);
...
static void define_path2d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_path2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Path2D"] = class_id;
	classes_by_id[class_id] = "Path2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &path2d_class_def);
...
void __register_path2d() {
	js_init_path2d_module(js_context());
}
