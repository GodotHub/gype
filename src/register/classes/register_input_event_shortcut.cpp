...
static void input_event_shortcut_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["InputEventShortcut"];
...
static JSClassDef input_event_shortcut_class_def = {
    "InputEventShortcut",
    input_event_shortcut_class_finalizer
};
...
static JSValue input_event_shortcut_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    InputEventShortcut *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<InputEventShortcut *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(InputEventShortcut);
...
static void define_input_event_shortcut_enum(JSContext *ctx, JSValue ctor) {
}

static int js_input_event_shortcut_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["InputEventShortcut"] = class_id;
	classes_by_id[class_id] = "InputEventShortcut";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &input_event_shortcut_class_def);
	...
	define_input_event_shortcut_enum(ctx, ctor);
...
void __register_input_event_shortcut() {
	js_init_input_event_shortcut_module(js_context());
}

void register_input_event_shortcut() {
	__register_input_event_shortcut();
}