...
static void visual_shader_node_multiply_add_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeMultiplyAdd"];
...
static JSClassDef visual_shader_node_multiply_add_class_def = {
    ...
    visual_shader_node_multiply_add_class_finalizer
};
...
static JSValue visual_shader_node_multiply_add_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeMultiplyAdd *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeMultiplyAdd *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeMultiplyAdd);
...
static JSValue visual_shader_node_multiply_add_class_set_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeMultiplyAdd::set_op_type, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_multiply_add_class_get_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeMultiplyAdd::get_op_type, ctx, this_val, argc, argv);
}
...
static void define_visual_shader_node_multiply_add_property(JSContext *ctx, JSValue proto) {
    JS_DefinePropertyGetSet(
        ctx,
        proto,
        JS_NewAtom(ctx, "op_type"),
        JS_NewCFunction(ctx, visual_shader_node_multiply_add_class_get_op_type, "get_op_type", 0),
        JS_NewCFunction(ctx, visual_shader_node_multiply_add_class_set_op_type, "set_op_type", 1),
        JS_PROP_GETSET
    );
}

static void define_visual_shader_node_multiply_add_enum(JSContext *ctx, JSValue proto) {
...
static int js_visual_shader_node_multiply_add_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeMultiplyAdd"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeMultiplyAdd";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_multiply_add_class_def);
	...
	define_visual_shader_node_multiply_add_property(ctx, proto);
	define_visual_shader_node_multiply_add_enum(ctx, proto);
...
void __register_visual_shader_node_multiply_add() {
	js_init_visual_shader_node_multiply_add_module(js_context());
}

void register_visual_shader_node_multiply_add() {
	__register_visual_shader_node_multiply_add();
}