...
static JSClassDef gltf_document_extension_convert_importer_mesh_class_def = {
    ...
    gltf_document_extension_convert_importer_mesh_class_finalizer
};
...
static int js_gltf_document_extension_convert_importer_mesh_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["GLTFDocumentExtensionConvertImporterMesh"] = class_id;
	classes_by_id[class_id] = "GLTFDocumentExtensionConvertImporterMesh";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &gltf_document_extension_convert_importer_mesh_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["GLTFDocumentExtension"]);
...
void __register_gltf_document_extension_convert_importer_mesh() {
	js_init_gltf_document_extension_convert_importer_mesh_module(js_context());
}

void register_gltf_document_extension_convert_importer_mesh() {
	__register_gltf_document_extension_convert_importer_mesh();
}