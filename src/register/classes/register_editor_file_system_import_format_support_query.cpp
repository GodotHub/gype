...
static void editor_file_system_import_format_support_query_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorFileSystemImportFormatSupportQuery"];
...
static JSClassDef editor_file_system_import_format_support_query_class_def = {
    ...
    editor_file_system_import_format_support_query_class_finalizer
};
...
static JSValue editor_file_system_import_format_support_query_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorFileSystemImportFormatSupportQuery *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorFileSystemImportFormatSupportQuery *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorFileSystemImportFormatSupportQuery);
...
static void define_editor_file_system_import_format_support_query_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_file_system_import_format_support_query_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorFileSystemImportFormatSupportQuery"] = class_id;
	classes_by_id[class_id] = "EditorFileSystemImportFormatSupportQuery";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_file_system_import_format_support_query_class_def);
	...
	define_editor_file_system_import_format_support_query_enum(ctx, ctor);
...
void __register_editor_file_system_import_format_support_query() {
	js_init_editor_file_system_import_format_support_query_module(js_context());
}

void register_editor_file_system_import_format_support_query() {
	__register_editor_file_system_import_format_support_query();
}