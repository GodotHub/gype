...
static void hinge_joint3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["HingeJoint3D"];
...
static JSValue hinge_joint3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    HingeJoint3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<HingeJoint3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(HingeJoint3D);
...
static int js_hinge_joint3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["HingeJoint3D"] = class_id;
	classes_by_id[class_id] = "HingeJoint3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &hinge_joint3d_class_def);
...
void __register_hinge_joint3d() {
	js_init_hinge_joint3d_module(js_context());
}

void register_hinge_joint3d() {
	__register_hinge_joint3d();
}