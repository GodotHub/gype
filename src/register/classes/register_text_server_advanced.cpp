...
static void text_server_advanced_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["TextServerAdvanced"];
...
static JSClassDef text_server_advanced_class_def = {
    "TextServerAdvanced",
    text_server_advanced_class_finalizer
};
...
static JSValue text_server_advanced_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    TextServerAdvanced *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<TextServerAdvanced *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(TextServerAdvanced);
...
static int js_text_server_advanced_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["TextServerAdvanced"] = class_id;
	classes_by_id[class_id] = "TextServerAdvanced";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &text_server_advanced_class_def);
...
void __register_text_server_advanced() {
	js_init_text_server_advanced_module(js_context());
}

void register_text_server_advanced() {
	__register_text_server_advanced();
}