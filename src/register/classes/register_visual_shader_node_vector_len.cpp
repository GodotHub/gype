...
static void visual_shader_node_vector_len_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeVectorLen"];
...
static JSClassDef visual_shader_node_vector_len_class_def = {
    ...
    visual_shader_node_vector_len_class_finalizer
};
...
static JSValue visual_shader_node_vector_len_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeVectorLen *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeVectorLen *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeVectorLen);
...
static void define_visual_shader_node_vector_len_enum(JSContext *ctx, JSValue ctor) {
}

static int js_visual_shader_node_vector_len_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeVectorLen"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeVectorLen";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_vector_len_class_def);
	...
	define_visual_shader_node_vector_len_enum(ctx, ctor);
...
void __register_visual_shader_node_vector_len() {
	js_init_visual_shader_node_vector_len_module(js_context());
}

void register_visual_shader_node_vector_len() {
	__register_visual_shader_node_vector_len();
}