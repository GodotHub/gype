...
static void prism_mesh_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PrismMesh"];
...
static JSValue prism_mesh_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<PrismMesh *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PrismMesh);
...
static JSValue prism_mesh_class_get_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		PrismMesh *obj = static_cast<PrismMesh *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_size();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		PrismMesh *js_proxy = static_cast<PrismMesh *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_prism_mesh_enum(JSContext *ctx, JSValue ctor) {
}

static int js_prism_mesh_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PrismMesh"] = class_id;
	classes_by_id[class_id] = "PrismMesh";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &prism_mesh_class_def);
	...
	define_prism_mesh_enum(ctx, ctor);
...
void __register_prism_mesh() {
	js_init_prism_mesh_module(js_context());
}

void register_prism_mesh() {
	__register_prism_mesh();
}