...
static void animation_node_blend2_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AnimationNodeBlend2"];
...
static JSClassDef animation_node_blend2_class_def = {
    "AnimationNodeBlend2",
    animation_node_blend2_class_finalizer
};
...
static JSValue animation_node_blend2_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AnimationNodeBlend2 *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AnimationNodeBlend2 *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AnimationNodeBlend2);
...
static void define_animation_node_blend2_enum(JSContext *ctx, JSValue ctor) {
}

static int js_animation_node_blend2_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AnimationNodeBlend2"] = class_id;
	classes_by_id[class_id] = "AnimationNodeBlend2";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &animation_node_blend2_class_def);
	...
	define_animation_node_blend2_enum(ctx, ctor);
...
void __register_animation_node_blend2() {
	js_init_animation_node_blend2_module(js_context());
}

void register_animation_node_blend2() {
	__register_animation_node_blend2();
}