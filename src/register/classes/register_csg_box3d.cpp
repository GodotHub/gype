...
static void csg_box3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["CSGBox3D"];
...
static JSValue csg_box3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<CSGBox3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(CSGBox3D);
...
static JSValue csg_box3d_class_get_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		CSGBox3D *obj = static_cast<CSGBox3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_size();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		CSGBox3D *js_proxy = static_cast<CSGBox3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_csg_box3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_csg_box3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["CSGBox3D"] = class_id;
	classes_by_id[class_id] = "CSGBox3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &csg_box3d_class_def);
	...
	define_csg_box3d_enum(ctx, ctor);
...
void __register_csg_box3d() {
	js_init_csg_box3d_module(js_context());
}

void register_csg_box3d() {
	__register_csg_box3d();
}