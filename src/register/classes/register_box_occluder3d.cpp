...
static void box_occluder3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["BoxOccluder3D"];
...
static JSClassDef box_occluder3d_class_def = {
    "BoxOccluder3D",
    box_occluder3d_class_finalizer
};
...
static JSValue box_occluder3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    BoxOccluder3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<BoxOccluder3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(BoxOccluder3D);
...
static JSValue box_occluder3d_class_get_size(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		BoxOccluder3D *obj = static_cast<BoxOccluder3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_size();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		BoxOccluder3D *js_proxy = static_cast<BoxOccluder3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_box_occluder3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_box_occluder3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["BoxOccluder3D"] = class_id;
	classes_by_id[class_id] = "BoxOccluder3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &box_occluder3d_class_def);
	...
	define_box_occluder3d_enum(ctx, ctor);
...
void __register_box_occluder3d() {
	js_init_box_occluder3d_module(js_context());
}

void register_box_occluder3d() {
	__register_box_occluder3d();
}