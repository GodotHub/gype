...
static void physical_bone_simulator3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PhysicalBoneSimulator3D"];
...
static JSClassDef physical_bone_simulator3d_class_def = {
    ...
    physical_bone_simulator3d_class_finalizer
};
...
static JSValue physical_bone_simulator3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PhysicalBoneSimulator3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PhysicalBoneSimulator3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PhysicalBoneSimulator3D);
...
static void define_physical_bone_simulator3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_physical_bone_simulator3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PhysicalBoneSimulator3D"] = class_id;
	classes_by_id[class_id] = "PhysicalBoneSimulator3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &physical_bone_simulator3d_class_def);
	...
	define_physical_bone_simulator3d_enum(ctx, ctor);
...
void __register_physical_bone_simulator3d() {
	js_init_physical_bone_simulator3d_module(js_context());
}

void register_physical_bone_simulator3d() {
	__register_physical_bone_simulator3d();
}