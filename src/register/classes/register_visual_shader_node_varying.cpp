...
static JSValue visual_shader_node_varying_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeVarying *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeVarying *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue visual_shader_node_varying_class_get_varying_name(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		VisualShaderNodeVarying *obj = static_cast<VisualShaderNodeVarying *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_varying_name();
	};
	proxy->setter = [this_val](const String &value) -> void {
		VisualShaderNodeVarying *js_proxy = static_cast<VisualShaderNodeVarying *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_varying_name(value);
	};
