...
static void image_texture_layered_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ImageTextureLayered"];
...
static JSClassDef image_texture_layered_class_def = {
    "ImageTextureLayered",
    image_texture_layered_class_finalizer
};
...
static JSValue image_texture_layered_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ImageTextureLayered *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ImageTextureLayered *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ImageTextureLayered);
...
static void define_image_texture_layered_enum(JSContext *ctx, JSValue ctor) {
}

static int js_image_texture_layered_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ImageTextureLayered"] = class_id;
	classes_by_id[class_id] = "ImageTextureLayered";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &image_texture_layered_class_def);
	...
	define_image_texture_layered_enum(ctx, ctor);
...
void __register_image_texture_layered() {
	js_init_image_texture_layered_module(js_context());
}

void register_image_texture_layered() {
	__register_image_texture_layered();
}