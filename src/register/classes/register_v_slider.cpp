...
static void v_slider_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VSlider"];
...
static JSValue v_slider_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<VSlider *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VSlider);
...
static void define_v_slider_enum(JSContext *ctx, JSValue ctor) {
}

static int js_v_slider_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VSlider"] = class_id;
	classes_by_id[class_id] = "VSlider";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &v_slider_class_def);
...
void __register_v_slider() {
	js_init_v_slider_module(js_context());
}
