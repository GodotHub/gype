...
static void packet_peer_extension_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PacketPeerExtension"];
...
static JSClassDef packet_peer_extension_class_def = {
    "PacketPeerExtension",
    packet_peer_extension_class_finalizer
};
...
static JSValue packet_peer_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PacketPeerExtension *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PacketPeerExtension *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PacketPeerExtension);
...
static void define_packet_peer_extension_enum(JSContext *ctx, JSValue ctor) {
}

static int js_packet_peer_extension_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PacketPeerExtension"] = class_id;
	classes_by_id[class_id] = "PacketPeerExtension";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &packet_peer_extension_class_def);
	...
	define_packet_peer_extension_enum(ctx, ctor);
...
void __register_packet_peer_extension() {
	js_init_packet_peer_extension_module(js_context());
}

void register_packet_peer_extension() {
	__register_packet_peer_extension();
}