...
static void visual_shader_node_transform_parameter_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeTransformParameter"];
...
static JSClassDef visual_shader_node_transform_parameter_class_def = {
    ...
    visual_shader_node_transform_parameter_class_finalizer
};
...
static JSValue visual_shader_node_transform_parameter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeTransformParameter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeTransformParameter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeTransformParameter);
...
static JSValue visual_shader_node_transform_parameter_class_set_default_value_enabled(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeTransformParameter::set_default_value_enabled, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_transform_parameter_class_is_default_value_enabled(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeTransformParameter::is_default_value_enabled, ctx, this_val, argc, argv);
}
static JSValue visual_shader_node_transform_parameter_class_set_default_value(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&VisualShaderNodeTransformParameter::set_default_value, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_transform_parameter_class_get_default_value(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Transform3D {
		VisualShaderNodeTransformParameter *obj = static_cast<VisualShaderNodeTransformParameter *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_default_value();
	};
	proxy->setter = [this_val](const Transform3D &value) -> void {
		VisualShaderNodeTransformParameter *js_proxy = static_cast<VisualShaderNodeTransformParameter *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_visual_shader_node_transform_parameter_property(JSContext *ctx, JSValue proto) {
    JS_DefinePropertyGetSet(
        ctx,
        proto,
        JS_NewAtom(ctx, "default_value_enabled"),
        JS_NewCFunction(ctx, visual_shader_node_transform_parameter_class_is_default_value_enabled, "is_default_value_enabled", 0),
        JS_NewCFunction(ctx, visual_shader_node_transform_parameter_class_set_default_value_enabled, "set_default_value_enabled", 1),
        JS_PROP_GETSET
    );
    JS_DefinePropertyGetSet(
        ctx,
        ...
        JS_NewCFunction(ctx, visual_shader_node_transform_parameter_class_get_default_value, "get_default_value", 0),
        JS_NewCFunction(ctx, visual_shader_node_transform_parameter_class_set_default_value, "set_default_value", 1),
        JS_PROP_GETSET
    );
}

static void define_visual_shader_node_transform_parameter_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_transform_parameter_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeTransformParameter"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeTransformParameter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_transform_parameter_class_def);
	...
	define_visual_shader_node_transform_parameter_property(ctx, proto);
	define_visual_shader_node_transform_parameter_enum(ctx, proto);
...
void __register_visual_shader_node_transform_parameter() {
	js_init_visual_shader_node_transform_parameter_module(js_context());
}

void register_visual_shader_node_transform_parameter() {
	__register_visual_shader_node_transform_parameter();
}