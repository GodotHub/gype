...
static void physics_point_query_parameters3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PhysicsPointQueryParameters3D"];
...
static JSClassDef physics_point_query_parameters3d_class_def = {
    ...
    physics_point_query_parameters3d_class_finalizer
};
...
static JSValue physics_point_query_parameters3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PhysicsPointQueryParameters3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PhysicsPointQueryParameters3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PhysicsPointQueryParameters3D);
...
static JSValue physics_point_query_parameters3d_class_get_position(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		PhysicsPointQueryParameters3D *obj = static_cast<PhysicsPointQueryParameters3D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_position();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		PhysicsPointQueryParameters3D *js_proxy = static_cast<PhysicsPointQueryParameters3D *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_physics_point_query_parameters3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_physics_point_query_parameters3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PhysicsPointQueryParameters3D"] = class_id;
	classes_by_id[class_id] = "PhysicsPointQueryParameters3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &physics_point_query_parameters3d_class_def);
	...
	define_physics_point_query_parameters3d_enum(ctx, ctor);
...
void __register_physics_point_query_parameters3d() {
	js_init_physics_point_query_parameters3d_module(js_context());
}

void register_physics_point_query_parameters3d() {
	__register_physics_point_query_parameters3d();
}