...
static void input_event_joypad_motion_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["InputEventJoypadMotion"];
...
static JSClassDef input_event_joypad_motion_class_def = {
    ...
    input_event_joypad_motion_class_finalizer
};
...
static JSValue input_event_joypad_motion_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    InputEventJoypadMotion *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<InputEventJoypadMotion *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(InputEventJoypadMotion);
...
static void define_input_event_joypad_motion_enum(JSContext *ctx, JSValue ctor) {
}

static int js_input_event_joypad_motion_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["InputEventJoypadMotion"] = class_id;
	classes_by_id[class_id] = "InputEventJoypadMotion";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &input_event_joypad_motion_class_def);
	...
	define_input_event_joypad_motion_enum(ctx, ctor);
...
void __register_input_event_joypad_motion() {
	js_init_input_event_joypad_motion_module(js_context());
}

void register_input_event_joypad_motion() {
	__register_input_event_joypad_motion();
}