...
static void engine_profiler_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EngineProfiler"];
...
static JSClassDef engine_profiler_class_def = {
    "EngineProfiler",
    engine_profiler_class_finalizer
};
...
static JSValue engine_profiler_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EngineProfiler *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EngineProfiler *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EngineProfiler);
...
static void define_engine_profiler_enum(JSContext *ctx, JSValue ctor) {
}

static int js_engine_profiler_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EngineProfiler"] = class_id;
	classes_by_id[class_id] = "EngineProfiler";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &engine_profiler_class_def);
	...
	define_engine_profiler_enum(ctx, ctor);
...
void __register_engine_profiler() {
	js_init_engine_profiler_module(js_context());
}

void register_engine_profiler() {
	__register_engine_profiler();
}