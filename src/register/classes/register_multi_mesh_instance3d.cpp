...
static void multi_mesh_instance3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["MultiMeshInstance3D"];
...
static JSClassDef multi_mesh_instance3d_class_def = {
    "MultiMeshInstance3D",
    multi_mesh_instance3d_class_finalizer
};
...
static JSValue multi_mesh_instance3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<MultiMeshInstance3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(MultiMeshInstance3D);
...
static JSValue multi_mesh_instance3d_class_set_multimesh(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&MultiMeshInstance3D::set_multimesh, ctx, this_val, argc, argv);
};
...
static void define_multi_mesh_instance3d_property(JSContext *ctx, JSValue proto) {
...
static int js_multi_mesh_instance3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["MultiMeshInstance3D"] = class_id;
	classes_by_id[class_id] = "MultiMeshInstance3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &multi_mesh_instance3d_class_def);
...
void __register_multi_mesh_instance3d() {
	js_init_multi_mesh_instance3d_module(js_context());
}

void register_multi_mesh_instance3d() {
	__register_multi_mesh_instance3d();
}