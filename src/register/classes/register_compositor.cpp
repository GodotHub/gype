...
static void compositor_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Compositor"];
...
static JSValue compositor_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Compositor *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Compositor);
...
static void define_compositor_enum(JSContext *ctx, JSValue ctor) {
}

static int js_compositor_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Compositor"] = class_id;
	classes_by_id[class_id] = "Compositor";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &compositor_class_def);
	...
	define_compositor_enum(ctx, ctor);
...
void __register_compositor() {
	js_init_compositor_module(js_context());
}

void register_compositor() {
	__register_compositor();
}