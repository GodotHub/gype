...
static JSValue collision_shape3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<CollisionShape3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_collision_shape3d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_collision_shape3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_collision_shape3d_enum(ctx, proto);
