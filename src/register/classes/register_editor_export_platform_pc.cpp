...
static void editor_export_platform_pc_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorExportPlatformPC"];
...
static JSClassDef editor_export_platform_pc_class_def = {
    ...
    editor_export_platform_pc_class_finalizer
};
...
static JSValue editor_export_platform_pc_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorExportPlatformPC *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorExportPlatformPC *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorExportPlatformPC);
...
static void define_editor_export_platform_pc_enum(JSContext *ctx, JSValue ctor) {
}

static int js_editor_export_platform_pc_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorExportPlatformPC"] = class_id;
	classes_by_id[class_id] = "EditorExportPlatformPC";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_export_platform_pc_class_def);
	...
	define_editor_export_platform_pc_enum(ctx, ctor);
...
void __register_editor_export_platform_pc() {
	js_init_editor_export_platform_pc_module(js_context());
}

void register_editor_export_platform_pc() {
	__register_editor_export_platform_pc();
}