...
static void audio_effect_eq21_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioEffectEQ21"];
...
static JSClassDef audio_effect_eq21_class_def = {
    "AudioEffectEQ21",
    audio_effect_eq21_class_finalizer
};
...
static JSValue audio_effect_eq21_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioEffectEQ21 *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioEffectEQ21 *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioEffectEQ21);
...
static void define_audio_effect_eq21_enum(JSContext *ctx, JSValue ctor) {
}

static int js_audio_effect_eq21_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["AudioEffectEQ21"] = class_id;
	classes_by_id[class_id] = "AudioEffectEQ21";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_effect_eq21_class_def);
	...
	define_audio_effect_eq21_enum(ctx, ctor);
...
void __register_audio_effect_eq21() {
	js_init_audio_effect_eq21_module(js_context());
}

void register_audio_effect_eq21() {
	__register_audio_effect_eq21();
}