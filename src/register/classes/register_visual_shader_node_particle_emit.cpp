...
static void visual_shader_node_particle_emit_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeParticleEmit"];
...
static JSClassDef visual_shader_node_particle_emit_class_def = {
    ...
    visual_shader_node_particle_emit_class_finalizer
};
...
static JSValue visual_shader_node_particle_emit_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeParticleEmit *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeParticleEmit *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeParticleEmit);
...
static int js_visual_shader_node_particle_emit_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisualShaderNodeParticleEmit"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeParticleEmit";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_particle_emit_class_def);
...
void __register_visual_shader_node_particle_emit() {
	js_init_visual_shader_node_particle_emit_module(js_context());
}

void register_visual_shader_node_particle_emit() {
	__register_visual_shader_node_particle_emit();
}