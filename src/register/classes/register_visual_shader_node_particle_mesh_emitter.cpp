...
static JSValue visual_shader_node_particle_mesh_emitter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeParticleMeshEmitter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeParticleMeshEmitter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_visual_shader_node_particle_mesh_emitter_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_visual_shader_node_particle_mesh_emitter_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	define_visual_shader_node_particle_mesh_emitter_enum(ctx, proto);
