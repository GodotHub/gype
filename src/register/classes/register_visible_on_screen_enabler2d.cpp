...
static void visible_on_screen_enabler2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisibleOnScreenEnabler2D"];
...
static JSClassDef visible_on_screen_enabler2d_class_def = {
    ...
    visible_on_screen_enabler2d_class_finalizer
};
...
static JSValue visible_on_screen_enabler2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisibleOnScreenEnabler2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisibleOnScreenEnabler2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisibleOnScreenEnabler2D);
...
static JSValue visible_on_screen_enabler2d_class_set_enable_mode(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisibleOnScreenEnabler2D::set_enable_mode, ctx, this_val, argc, argv);
};
...
static JSValue visible_on_screen_enabler2d_class_set_enable_node_path(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&VisibleOnScreenEnabler2D::set_enable_node_path, ctx, this_val, argc, argv);
};
...
static void define_visible_on_screen_enabler2d_property(JSContext *ctx, JSValue proto) {
...
static void define_visible_on_screen_enabler2d_enum(JSContext *ctx, JSValue proto) {
...
static int js_visible_on_screen_enabler2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VisibleOnScreenEnabler2D"] = class_id;
	classes_by_id[class_id] = "VisibleOnScreenEnabler2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visible_on_screen_enabler2d_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["VisibleOnScreenNotifier2D"]);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, class_id, proto);

	define_visible_on_screen_enabler2d_property(ctx, proto);
	define_visible_on_screen_enabler2d_enum(ctx, proto);
...
void __register_visible_on_screen_enabler2d() {
	js_init_visible_on_screen_enabler2d_module(js_context());
}

void register_visible_on_screen_enabler2d() {
	__register_visible_on_screen_enabler2d();
}