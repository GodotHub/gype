...
static void placeholder_mesh_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["PlaceholderMesh"];
...
static JSClassDef placeholder_mesh_class_def = {
    "PlaceholderMesh",
    placeholder_mesh_class_finalizer
};
...
static JSValue placeholder_mesh_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    PlaceholderMesh *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<PlaceholderMesh *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(PlaceholderMesh);
...
static void define_placeholder_mesh_enum(JSContext *ctx, JSValue ctor) {
}

static int js_placeholder_mesh_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["PlaceholderMesh"] = class_id;
	classes_by_id[class_id] = "PlaceholderMesh";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &placeholder_mesh_class_def);
	...
	define_placeholder_mesh_enum(ctx, ctor);
...
void __register_placeholder_mesh() {
	js_init_placeholder_mesh_module(js_context());
}

void register_placeholder_mesh() {
	__register_placeholder_mesh();
}