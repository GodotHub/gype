...
static void audio_listener3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["AudioListener3D"];
...
static JSClassDef audio_listener3d_class_def = {
    "AudioListener3D",
    audio_listener3d_class_finalizer
};
...
static JSValue audio_listener3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    AudioListener3D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<AudioListener3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(AudioListener3D);
...
static JSValue audio_listener3d_class_set_doppler_tracking(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&AudioListener3D::set_doppler_tracking, ctx, this_val, argc, argv);
};
static JSValue audio_listener3d_class_get_doppler_tracking(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
...
static const JSCFunctionListEntry audio_listener3d_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("set_doppler_tracking", 1, &audio_listener3d_class_set_doppler_tracking),
	JS_CFUNC_DEF("get_doppler_tracking", 0, &audio_listener3d_class_get_doppler_tracking),
};
...
static void define_audio_listener3d_property(JSContext *ctx, JSValue proto) {
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, audio_listener3d_class_get_doppler_tracking, "get_doppler_tracking", 0),
        JS_NewCFunction(ctx, audio_listener3d_class_set_doppler_tracking, "set_doppler_tracking", 1),
        JS_PROP_GETSET
    );
}
...
static int js_audio_listener3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["AudioListener3D"] = class_id;
	classes_by_id[class_id] = "AudioListener3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &audio_listener3d_class_def);
...
void __register_audio_listener3d() {
	js_init_audio_listener3d_module(js_context());
}

void register_audio_listener3d() {
	__register_audio_listener3d();
}