...
static JSValue cubemap_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Cubemap *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_cubemap_enum(JSContext *ctx, JSValue proto) {
}
