...
static JSValue dtls_server_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<DTLSServer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(DTLSServer);
...
static void define_dtls_server_enum(JSContext *ctx, JSValue ctor) {
}

static int js_dtls_server_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["DTLSServer"] = class_id;
	classes_by_id[class_id] = "DTLSServer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &dtls_server_class_def);
	...
	define_dtls_server_enum(ctx, ctor);
...
void __register_dtls_server() {
	js_init_dtls_server_module(js_context());
}

void register_dtls_server() {
	__register_dtls_server();
}