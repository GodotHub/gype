...
static void visual_shader_node_vec3_constant_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VisualShaderNodeVec3Constant"];
...
static JSClassDef visual_shader_node_vec3_constant_class_def = {
    ...
    visual_shader_node_vec3_constant_class_finalizer
};
...
static JSValue visual_shader_node_vec3_constant_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VisualShaderNodeVec3Constant *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VisualShaderNodeVec3Constant *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VisualShaderNodeVec3Constant);
...
static JSValue visual_shader_node_vec3_constant_class_set_constant(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    return call_builtin_method_no_ret(&VisualShaderNodeVec3Constant::set_constant, ctx, this_val, argc, argv);
};
static JSValue visual_shader_node_vec3_constant_class_get_constant(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector3 {
		VisualShaderNodeVec3Constant *obj = static_cast<VisualShaderNodeVec3Constant *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_constant();
	};
	proxy->setter = [this_val](const Vector3 &value) -> void {
		VisualShaderNodeVec3Constant *js_proxy = static_cast<VisualShaderNodeVec3Constant *>(VariantAdapter(this_val).get().operator Object *());
...
static void define_visual_shader_node_vec3_constant_property(JSContext *ctx, JSValue proto) {
...
static void define_visual_shader_node_vec3_constant_enum(JSContext *ctx, JSValue proto) {
}

static int js_visual_shader_node_vec3_constant_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["VisualShaderNodeVec3Constant"] = class_id;
	classes_by_id[class_id] = "VisualShaderNodeVec3Constant";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &visual_shader_node_vec3_constant_class_def);
	...
	define_visual_shader_node_vec3_constant_enum(ctx, proto);
...
void __register_visual_shader_node_vec3_constant() {
	js_init_visual_shader_node_vec3_constant_module(js_context());
}

void register_visual_shader_node_vec3_constant() {
	__register_visual_shader_node_vec3_constant();
}