...
static JSValue time_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	JSClassID class_id = classes["_Time"];
	...
	VariantAdapter *adapter = memnew(VariantAdapter(time_class));
...
static int js_time_class_init(JSContext *ctx, JSModuleDef *m){
	...
	JS_SetModuleExport(ctx, m, "Time", singleton);
...
void register_time() {
	_js_init_time_module(js_context(), "@godot/classes/time");
}