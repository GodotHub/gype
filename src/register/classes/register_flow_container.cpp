...
static void flow_container_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["FlowContainer"];
...
static JSClassDef flow_container_class_def = {
    "FlowContainer",
    flow_container_class_finalizer
};
...
static JSValue flow_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    FlowContainer *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<FlowContainer *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(FlowContainer);
...
static void define_flow_container_enum(JSContext *ctx, JSValue ctor) {
	...
	JS_SetPropertyStr(ctx, ctor, "LastWrapAlignmentMode", LastWrapAlignmentMode_obj);
}

static int js_flow_container_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["FlowContainer"] = class_id;
	classes_by_id[class_id] = "FlowContainer";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &flow_container_class_def);
	...
	define_flow_container_enum(ctx, ctor);
...
void __register_flow_container() {
	js_init_flow_container_module(js_context());
}

void register_flow_container() {
	__register_flow_container();
}