...
static void editor_export_platform_linux_bsd_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorExportPlatformLinuxBSD"];
...
static JSClassDef editor_export_platform_linux_bsd_class_def = {
    ...
    editor_export_platform_linux_bsd_class_finalizer
};
...
static JSValue editor_export_platform_linux_bsd_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorExportPlatformLinuxBSD *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorExportPlatformLinuxBSD *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorExportPlatformLinuxBSD);
...
static void define_editor_export_platform_linux_bsd_property(JSContext *ctx, JSValue proto) {
}

static void define_editor_export_platform_linux_bsd_enum(JSContext *ctx, JSValue proto) {
}

static int js_editor_export_platform_linux_bsd_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorExportPlatformLinuxBSD"] = class_id;
	classes_by_id[class_id] = "EditorExportPlatformLinuxBSD";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_export_platform_linux_bsd_class_def);
	...
	define_editor_export_platform_linux_bsd_enum(ctx, proto);
...
void __register_editor_export_platform_linux_bsd() {
	js_init_editor_export_platform_linux_bsd_module(js_context());
}

void register_editor_export_platform_linux_bsd() {
	__register_editor_export_platform_linux_bsd();
}