...
static JSClassDef editor_scene_format_importer_class_def = {
    ...
    editor_scene_format_importer_class_finalizer
};
...
static JSValue editor_scene_format_importer_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorSceneFormatImporter *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorSceneFormatImporter *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorSceneFormatImporter);
...
static JSValue editor_scene_format_importer_class_add_import_option(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
    return call_builtin_method_no_ret(&EditorSceneFormatImporter::add_import_option, ctx, this_val, argc, argv);
};
static JSValue editor_scene_format_importer_class_add_import_option_advanced(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
...
static const JSCFunctionListEntry editor_scene_format_importer_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("add_import_option_advanced", 6, &editor_scene_format_importer_class_add_import_option_advanced),
};




static void define_editor_scene_format_importer_property(JSContext *ctx, JSValue proto) {
}

static void define_editor_scene_format_importer_enum(JSContext *ctx, JSValue proto) {
}

static int js_editor_scene_format_importer_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorSceneFormatImporter"] = class_id;
	classes_by_id[class_id] = "EditorSceneFormatImporter";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_scene_format_importer_class_def);
	...
	define_editor_scene_format_importer_enum(ctx, proto);
	JS_SetPropertyFunctionList(ctx, proto, editor_scene_format_importer_class_proto_funcs, _countof(editor_scene_format_importer_class_proto_funcs));
...
void __register_editor_scene_format_importer() {
	js_init_editor_scene_format_importer_module(js_context());
}

void register_editor_scene_format_importer() {
	__register_editor_scene_format_importer();
}