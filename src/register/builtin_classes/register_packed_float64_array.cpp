...
static void packed_float64_array_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes[typeid(PackedFloat64Array)];
	GDVariantAdapter<PackedFloat64Array> *opaque_ptr = static_cast<GDVariantAdapter<PackedFloat64Array> *>(JS_GetOpaque(val, class_id));
	if (opaque_ptr && opaque_ptr->can_memfree) {
		memfree(const_cast<PackedFloat64Array *>(opaque_ptr->m_active_variant));
	}
}
...
static JSValue packed_float64_array_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	GDVariantAdapter<PackedFloat64Array> *adapter = reinterpret_cast<GDVariantAdapter<PackedFloat64Array> *>(memalloc(sizeof(GDVariantAdapter<PackedFloat64Array>)));
	if (argc == 0) {
		instance = reinterpret_cast<PackedFloat64Array *>(memalloc(sizeof(PackedFloat64Array)));
		instance = new (instance) PackedFloat64Array();
	}
	if (argc == 1&&(JSValueAdapter<PackedFloat64Array>::can_cast(argv[0]))) {
		...
		instance = reinterpret_cast<PackedFloat64Array *>(memalloc(sizeof(PackedFloat64Array)));
		instance = new (instance) PackedFloat64Array(v0);
	}
	...
	adapter = new (adapter) GDVariantAdapter<PackedFloat64Array>(*instance, true);
...
static int js_packed_float64_array_class_init(JSContext *ctx) {
	JSClassID class_id = JS_NewClassID(&classes[typeid(PackedFloat64Array)]);
