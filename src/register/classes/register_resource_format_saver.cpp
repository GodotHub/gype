...
static void resource_format_saver_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ResourceFormatSaver"];
...
static JSClassDef resource_format_saver_class_def = {
    "ResourceFormatSaver",
    resource_format_saver_class_finalizer
};
...
static JSValue resource_format_saver_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ResourceFormatSaver *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ResourceFormatSaver *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ResourceFormatSaver);
...
static void define_resource_format_saver_enum(JSContext *ctx, JSValue ctor) {
}

static int js_resource_format_saver_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ResourceFormatSaver"] = class_id;
	classes_by_id[class_id] = "ResourceFormatSaver";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &resource_format_saver_class_def);
	...
	define_resource_format_saver_enum(ctx, ctor);
...
void __register_resource_format_saver() {
	js_init_resource_format_saver_module(js_context());
}

void register_resource_format_saver() {
	__register_resource_format_saver();
}