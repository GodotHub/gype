...
static void quad_mesh_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["QuadMesh"];
...
static JSValue quad_mesh_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<QuadMesh *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(QuadMesh);
...
static int js_quad_mesh_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["QuadMesh"] = class_id;
	classes_by_id[class_id] = "QuadMesh";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &quad_mesh_class_def);

	JSValue proto = JS_NewObjectClass(ctx, class_id);
	JSValue base_class = JS_GetClassProto(ctx, classes["PlaneMesh"]);
...
void __register_quad_mesh() {
	js_init_quad_mesh_module(js_context());
}

void register_quad_mesh() {
	__register_quad_mesh();
}