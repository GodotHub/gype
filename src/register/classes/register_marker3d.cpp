...
static void marker3d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["Marker3D"];
...
static JSValue marker3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		...
		instance = static_cast<Marker3D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(Marker3D);
...
static void define_marker3d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_marker3d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Marker3D"] = class_id;
	classes_by_id[class_id] = "Marker3D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &marker3d_class_def);
...
void __register_marker3d() {
	js_init_marker3d_module(js_context());
}
