...
static JSClassDef texture2d_array_class_def = {
    "Texture2DArray",
    texture2d_array_class_finalizer
};
...
static void define_texture2d_array_enum(JSContext *ctx, JSValue ctor) {
}

static int js_texture2d_array_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["Texture2DArray"] = class_id;
	classes_by_id[class_id] = "Texture2DArray";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &texture2d_array_class_def);
...
void __register_texture2d_array() {
	js_init_texture2d_array_module(js_context());
}

void register_texture2d_array() {
	__register_texture2d_array();
}