...
static void editor_scene_format_importer_fbx2_gltf_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["EditorSceneFormatImporterFBX2GLTF"];
...
static JSClassDef editor_scene_format_importer_fbx2_gltf_class_def = {
    ...
    editor_scene_format_importer_fbx2_gltf_class_finalizer
};
...
static JSValue editor_scene_format_importer_fbx2_gltf_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorSceneFormatImporterFBX2GLTF *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorSceneFormatImporterFBX2GLTF *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(EditorSceneFormatImporterFBX2GLTF);
...
static void define_editor_scene_format_importer_fbx2_gltf_property(JSContext *ctx, JSValue proto) {
}

static void define_editor_scene_format_importer_fbx2_gltf_enum(JSContext *ctx, JSValue proto) {
}

static int js_editor_scene_format_importer_fbx2_gltf_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["EditorSceneFormatImporterFBX2GLTF"] = class_id;
	classes_by_id[class_id] = "EditorSceneFormatImporterFBX2GLTF";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &editor_scene_format_importer_fbx2_gltf_class_def);
	...
	define_editor_scene_format_importer_fbx2_gltf_enum(ctx, proto);
...
void __register_editor_scene_format_importer_fbx2_gltf() {
	js_init_editor_scene_format_importer_fbx2_gltf_module(js_context());
}

void register_editor_scene_format_importer_fbx2_gltf() {
	__register_editor_scene_format_importer_fbx2_gltf();
}