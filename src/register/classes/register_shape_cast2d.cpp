...
static void shape_cast2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ShapeCast2D"];
...
static JSValue shape_cast2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ShapeCast2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ShapeCast2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ShapeCast2D);
...
static JSValue shape_cast2d_class_get_target_position(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> Vector2 {
		ShapeCast2D *obj = static_cast<ShapeCast2D *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_target_position();
	};
	proxy->setter = [this_val](const Vector2 &value) -> void {
		ShapeCast2D *js_proxy = static_cast<ShapeCast2D *>(VariantAdapter(this_val).get().operator Object *());
...
static JSValue shape_cast2d_class_get_collision_result(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
...
static const JSCFunctionListEntry shape_cast2d_class_proto_funcs[] = {
	...
	JS_CFUNC_DEF("get_collision_result", 0, &shape_cast2d_class_get_collision_result),
};
...
static void define_shape_cast2d_property(JSContext *ctx, JSValue proto) {
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, shape_cast2d_class_get_collision_result, "get_collision_result", 0),
        JS_UNDEFINED,
        JS_PROP_GETSET
    );
    JS_DefinePropertyGetSet(
...
static void define_shape_cast2d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_shape_cast2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ShapeCast2D"] = class_id;
	classes_by_id[class_id] = "ShapeCast2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &shape_cast2d_class_def);
	...
	define_shape_cast2d_enum(ctx, ctor);
...
void __register_shape_cast2d() {
	js_init_shape_cast2d_module(js_context());
}

void register_shape_cast2d() {
	__register_shape_cast2d();
}