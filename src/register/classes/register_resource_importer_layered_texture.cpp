...
static void resource_importer_layered_texture_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ResourceImporterLayeredTexture"];
...
static JSClassDef resource_importer_layered_texture_class_def = {
    ...
    resource_importer_layered_texture_class_finalizer
};
...
static JSValue resource_importer_layered_texture_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ResourceImporterLayeredTexture *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ResourceImporterLayeredTexture *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ResourceImporterLayeredTexture);
...
static void define_resource_importer_layered_texture_property(JSContext *ctx, JSValue proto) {
}

static void define_resource_importer_layered_texture_enum(JSContext *ctx, JSValue proto) {
}

static int js_resource_importer_layered_texture_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ResourceImporterLayeredTexture"] = class_id;
	classes_by_id[class_id] = "ResourceImporterLayeredTexture";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &resource_importer_layered_texture_class_def);
	...
	define_resource_importer_layered_texture_enum(ctx, proto);
...
void __register_resource_importer_layered_texture() {
	js_init_resource_importer_layered_texture_module(js_context());
}

void register_resource_importer_layered_texture() {
	__register_resource_importer_layered_texture();
}