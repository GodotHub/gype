...
static JSValue editor_export_platform_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    EditorExportPlatform *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<EditorExportPlatform *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static void define_editor_export_platform_enum(JSContext *ctx, JSValue proto) {
	...
	JS_SetPropertyStr(ctx, proto, "ExportMessageType", ExportMessageType_obj);
	...
	JS_SetPropertyStr(ctx, proto, "DebugFlags", DebugFlags_obj);
}
