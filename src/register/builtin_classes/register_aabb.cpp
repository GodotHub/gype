...
static void aabb_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes[typeid(AABB)];
	GDVariantAdapter<AABB> *opaque_ptr = static_cast<GDVariantAdapter<AABB> *>(JS_GetOpaque(val, class_id));
	if (opaque_ptr && opaque_ptr->can_memfree) {
		memfree(const_cast<AABB *>(opaque_ptr->m_active_variant));
	}
}
...
static JSValue aabb_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	GDVariantAdapter<AABB> *adapter = reinterpret_cast<GDVariantAdapter<AABB> *>(memalloc(sizeof(GDVariantAdapter<AABB>)));
	if (argc == 0) {
		instance = reinterpret_cast<AABB *>(memalloc(sizeof(AABB)));
		instance = new (instance) AABB();
	}
	if (argc == 1 && (JSValueAdapter<AABB>::can_cast(argv[0]))) {
		AABB v0 = *JSValueAdapter<AABB>(argv[0]).get();
		instance = reinterpret_cast<AABB *>(memalloc(sizeof(AABB)));
		instance = new (instance) AABB(v0);
	}
	if (argc == 2 && (JSValueAdapter<Vector3>::can_cast(argv[0])) && (JSValueAdapter<Vector3>::can_cast(argv[1]))) {
		...
		instance = reinterpret_cast<AABB *>(memalloc(sizeof(AABB)));
		instance = new (instance) AABB(v0, v1);
	}
	adapter = new (adapter) GDVariantAdapter<AABB>(*instance, true);
