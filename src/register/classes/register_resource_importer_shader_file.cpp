...
static void resource_importer_shader_file_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["ResourceImporterShaderFile"];
...
static JSClassDef resource_importer_shader_file_class_def = {
    ...
    resource_importer_shader_file_class_finalizer
};
...
static JSValue resource_importer_shader_file_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    ResourceImporterShaderFile *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<ResourceImporterShaderFile *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(ResourceImporterShaderFile);
...
static void define_resource_importer_shader_file_enum(JSContext *ctx, JSValue ctor) {
}

static int js_resource_importer_shader_file_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["ResourceImporterShaderFile"] = class_id;
	classes_by_id[class_id] = "ResourceImporterShaderFile";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &resource_importer_shader_file_class_def);
	...
	define_resource_importer_shader_file_enum(ctx, ctor);
...
void __register_resource_importer_shader_file() {
	js_init_resource_importer_shader_file_module(js_context());
}

void register_resource_importer_shader_file() {
	__register_resource_importer_shader_file();
}