...
static void kinematic_collision2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["KinematicCollision2D"];
...
static JSClassDef kinematic_collision2d_class_def = {
    ...
    kinematic_collision2d_class_finalizer
};
...
static JSValue kinematic_collision2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    KinematicCollision2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<KinematicCollision2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(KinematicCollision2D);
...
static void define_kinematic_collision2d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_kinematic_collision2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["KinematicCollision2D"] = class_id;
	classes_by_id[class_id] = "KinematicCollision2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &kinematic_collision2d_class_def);
	...
	define_kinematic_collision2d_enum(ctx, ctor);
...
void __register_kinematic_collision2d() {
	js_init_kinematic_collision2d_module(js_context());
}

void register_kinematic_collision2d() {
	__register_kinematic_collision2d();
}