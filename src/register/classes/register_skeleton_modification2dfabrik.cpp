...
static JSValue skeleton_modification2dfabrik_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    SkeletonModification2DFABRIK *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<SkeletonModification2DFABRIK *>(VariantAdapter(*argv).get().operator Object *());
    } else {
...
static JSValue skeleton_modification2dfabrik_class_get_target_node(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> NodePath {
		SkeletonModification2DFABRIK *obj = static_cast<SkeletonModification2DFABRIK *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_target_node();
	};
	proxy->setter = [this_val](const NodePath &value) -> void {
		SkeletonModification2DFABRIK *js_proxy = static_cast<SkeletonModification2DFABRIK *>(VariantAdapter(this_val).get().operator Object *());
