...
static void video_stream_playback_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["VideoStreamPlayback"];
...
static JSClassDef video_stream_playback_class_def = {
    "VideoStreamPlayback",
    video_stream_playback_class_finalizer
};
...
static JSValue video_stream_playback_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    VideoStreamPlayback *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<VideoStreamPlayback *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(VideoStreamPlayback);
...
static int js_video_stream_playback_class_init(JSContext *ctx, JSModuleDef *m) {	
	...
	classes["VideoStreamPlayback"] = class_id;
	classes_by_id[class_id] = "VideoStreamPlayback";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &video_stream_playback_class_def);
...
void __register_video_stream_playback() {
	js_init_video_stream_playback_module(js_context());
}

void register_video_stream_playback() {
	__register_video_stream_playback();
}