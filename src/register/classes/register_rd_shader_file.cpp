...
static void rd_shader_file_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RDShaderFile"];
...
static JSClassDef rd_shader_file_class_def = {
    "RDShaderFile",
    rd_shader_file_class_finalizer
};
...
static JSValue rd_shader_file_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RDShaderFile *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RDShaderFile *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RDShaderFile);
...
static JSValue rd_shader_file_class_get_base_error(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	...
	proxy->getter = [this_val]() -> String {
		RDShaderFile *obj = static_cast<RDShaderFile *>(VariantAdapter(this_val).get().operator Object*());
		return obj->get_base_error();
	};
	proxy->setter = [this_val](const String &value) -> void {
		RDShaderFile *js_proxy = static_cast<RDShaderFile *>(VariantAdapter(this_val).get().operator Object *());
		js_proxy->set_base_error(value);
	};
...
static void define_rd_shader_file_enum(JSContext *ctx, JSValue ctor) {
}

static int js_rd_shader_file_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["RDShaderFile"] = class_id;
	classes_by_id[class_id] = "RDShaderFile";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &rd_shader_file_class_def);
	...
	define_rd_shader_file_enum(ctx, ctor);
...
void __register_rd_shader_file() {
	js_init_rd_shader_file_module(js_context());
}

void register_rd_shader_file() {
	__register_rd_shader_file();
}