...
static void remote_transform2d_class_finalizer(JSRuntime *rt, JSValue val) {
	JSClassID class_id = classes["RemoteTransform2D"];
...
static JSClassDef remote_transform2d_class_def = {
    "RemoteTransform2D",
    remote_transform2d_class_finalizer
};
...
static JSValue remote_transform2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
    ...
    RemoteTransform2D *instance;
	VariantAdapter *adapter;
	JSClassID opaque_id;
    // Allow constructing from an existing native pointer
    if (argc == 1 && VariantAdapter::can_cast(argv[0], Variant::Type::OBJECT)) {
		adapter = static_cast<VariantAdapter *>(JS_GetAnyOpaque(*argv, &opaque_id));
		instance = static_cast<RemoteTransform2D *>(VariantAdapter(*argv).get().operator Object *());
    } else {
        instance = memnew(RemoteTransform2D);
...
static void define_remote_transform2d_enum(JSContext *ctx, JSValue ctor) {
}

static int js_remote_transform2d_class_init(JSContext *ctx, JSModuleDef *m) {	
	JSClassID class_id = 0;
	class_id = JS_NewClassID(js_runtime(), &class_id);
	classes["RemoteTransform2D"] = class_id;
	classes_by_id[class_id] = "RemoteTransform2D";
	JS_NewClass(JS_GetRuntime(ctx), class_id, &remote_transform2d_class_def);
	...
	define_remote_transform2d_enum(ctx, ctor);
...
void __register_remote_transform2d() {
	js_init_remote_transform2d_module(js_context());
}

void register_remote_transform2d() {
	__register_remote_transform2d();
}